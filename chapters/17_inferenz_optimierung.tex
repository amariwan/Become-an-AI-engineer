...
\begin{praxishinweis}
TTFT und TPOT sind die zwei Metriken, die User wirklich spüren. TTFT $<$ 500ms für Streaming, TPOT $<$ 30ms pro Token. Misst immer den P95-Wert -- der Durchschnitt lügt bei Latenz.
\end{praxishinweis}
...
\begin{praxishinweis}
Continuous Batching ist kein magischer Turbo. Für interaktive Chat-Anwendungen kleine Batches (4-8) wählen, sonst leidet die TTFT. Für Batch-Jobs 32-64. Und immer \texttt{--enable-prefix-caching} aktivieren.
\end{praxishinweis}
...
\begin{description}[leftmargin=*,style=nextline]
    ...
    \item[AWQ (Activation-Aware)] Berücksichtigt die Aktivierungsverteilung, nicht nur die Gewichte. Erhält mehr Qualität als GPTQ bei gleicher Bitrate. Der empfohlene Standard für GPU-Inference.
...
\begin{praxishinweis}
Quantisierung nie blind deployen. Ein INT4-Modell verliert je nach Domäne 2--25\,\%. Golden Dataset auf FP16 und INT4 evaluieren -- erst dann entscheiden. Für Code und Mathematik lieber INT8 nehmen.
\end{praxishinweis}
...
\begin{praxishinweis}
Speculative Decoding bringt 2-3x Speedup -- aber nur wenn Draft und Target denselben Tokenizer teilen. Prüft vor dem Deployment die Akzeptanzrate: unter 60\,\% lohnt sich der Overhead nicht.
\end{praxishinweis}
...
\begin{praxishinweis}
Inference-Server sind kritische Infrastruktur. Immer hinter API-Gateway betreiben, Rate-Limiting aktivieren, Audit-Log führen. Und niemals \texttt{--gpu-memory-utilization 1.0} setzen -- der KV-Cache braucht Reserve für Spitzen.
\end{praxishinweis}
