...
% INTERVIEWFRAGEN
% ===========================================================%
\section{Interviewfragen -- Failure-Mode-Analyse}

\begin{enumerate}[leftmargin=*]
    \item \textbf{Dein RAG-System hat eine Halluzinationsrate von 12\,\%. Beschreibe deinen Debugging-Prozess in 5 Schritten.}

    \textbf{System-Review:} (1) Retrieval-Abdeckung prüfen (findet das Embedding relevante Chunks?). (2) Chunking-Strategie prüfen (zu grosse/kleine Chunks?). (3) Prompt-Struktur prüfen (wird \glqq weiss nicht\grqq{} erlaubt?). (4) Post-Retrieval-Reranking mit Cross-Encoder. (5) Golden Dataset aufbauen und Halluzinationen als Testfälle erfassen.

    \item \textbf{Ein Multi-Agent-System kostet plötzlich 4x so viel pro Task. Welche drei Komponenten prüfst du zuerst?}

    \textbf{Antwort:} (1) Agent-Step-Count (wie viele Tool-Calls pro Task? Ein Loop könnte stecken). (2) Token-Verbrauch pro Schritt (System-Prompts gewachsen?). (3) Modell-Wechsel (Provider hat Modell hinter generischem String ausgetauscht?). Debugging: Tracing-Logs nach steps/task gruppieren.

    \item \textbf{Du deployst einen Prompt-Update, der die Accuracy von 88\,\% auf 94\,\% erhöht -- aber die Cost pro Query steigt von 0,003 EUR auf 0,015 EUR. Wie entscheidest du?}

    \textbf{Antwort:} Kosten-Nutzen-Analyse: 6\,\% mehr Accuracy ist bei Support-Tickets (10.000/Tag) 600 korrekte Antworten mehr. Aber 0,012 EUR Mehrkosten pro Query = 120 EUR/Tag = 3.600 EUR/Monat. Ist die Accuracy-Steigerung 3.600 EUR wert? Alternative: Nur schwierige Queries ($>$ 0,7 Confidence) ans teurere Modell routen, einfache bleiben günstig.

    \item \textbf{Dein LLM-System fällt aus, weil der einzige Provider Rate-Limits hat. Du hast keine Fallback-Infrastruktur. Was machst du in den nächsten 5 Minuten?}

    \textbf{Antwort:} (1) Caching sofort auf maximale TTL stellen (alle Responses aus Cache bedienen, auch wenn veraltet). (2) Degraded Mode: Statische FAQ-Seite bereitstellen. (3) Provider-Status-Seite prüfen, Rate-Limit-Reset-Zeit ermitteln. (4) Backup-Provider-Integration starten (OpenAI-Client liegt bereit, muss nur API-Key aktivieren). Langfristig: Fallback-Chain als Baustein der Architektur etablieren.

    \item \textbf{Ein Entwickler behauptet: \glqq Unser System hat 99,9\,\% Availability.\grqq{} Was fragst du als nächstes?}

    \textbf{Antwort:} (1) Ist das die API-Verfügbarkeit oder die End-to-End-Verfügbarkeit? (2) Was ist das SLI -- erfolgreiche HTTP-Responses oder korrekte LLM-Antworten? (3) Wie wird gemessen -- synthetische Tests oder Produktionstraffic? (4) Fehler werden in 5 Minuten erkannt (MTTD) und in 30 Minuten behoben (MTTR)? Availability ohne Error-Budget und Incident-Response ist eine Kennzahl ohne Aussagekraft.

    \item \textbf{Dein Semantic Cache hat eine Hit-Rate von 8\,\%. Welche fünf Stellschrauben justierst du?}

    \textbf{Antwort:} (1) Threshold senken (0,95 $\rightarrow$ 0,90 -- mehr Hits, aber Risiko false positives prüfen). (2) Embedding-Modell wechseln (text-embedding-3-small vs text-embedding-3-large). (3) Cache-Key-Strategie ändern (Query normalisieren: Lowercase, Stopwords entfernen). (4) Intent-basiertes Caching (FAQ-Intents cachen, personalisierte nicht). (5) TTL erhöhen (1h $\rightarrow$ 24h für statische Inhalte).

    \item \textbf{Wie beweist du einem Interviewer in 3 Minuten, dass dein Portfolio-Projekt produktionsreif ist?}

    \textbf{Antwort:} Zeige drei Artefakte: (1) \texttt{docker-compose.yml} mit Healthcheck und mehreren Services. (2) \texttt{.github/workflows/eval.yml} mit Golden-Dataset-Check. (3) Ein Screenshot des Tracing-Dashboards mit Latenz-P95, Cost-per-Query und Halluzinationsrate. Das deckt Deployment, Eval und Observability in 3 Minuten ab.
\end{enumerate}
