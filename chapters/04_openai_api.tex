...
% ============================================================
\autornotiz{
2023 habe ich mein erstes LLM-System in Produktion deployed. OpenAI API, gpt-3.5-turbo, temperature 0.7, keine Validierung. Kosten: \$400/Tag. Accuracy: 61\%.
Heute: gpt-4o-mini, temperature 0.0, Structured Outputs (Strict JSON Schema), Semantic Caching, Prompt Registry mit Jinja2 + Git-Tags, Golden Set Evaluation in CI.
Kosten: \$12/Tag. Accuracy: 94\%.
Der Unterschied war nicht das Modell. Es war Disziplin bei der API-Schnittstelle.
}
...
\begin{itemize}
    ...
    \item \textbf{Seed} Ein Integer-Wert, der reproduzierbare Ergebnisse ermöglicht. Bei gleichem Seed, Modell und Parametern liefert die API weitgehend deterministische Ergebnisse -- allerdings ohne Garantie, da serverseitiges Batching die Floating-Point-Summationsreihenfolge beeinflussen kann. Wichtig für Tests und Regression.
    \item \textbf{Temperature} Steuert die Zufälligkeit der Token-Auswahl. 0.0 = weitgehend deterministisch (reduziert Varianz drastisch, garantiert aber keine Bit-Identität), 1.0 = maximal kreativ. Für Produktion: 0.0--0.2.
    \item \textbf{Top-P} Nucleus Sampling -- Alternative zu Temperature. Wähle Tokens, bis deren kumulative Wahrscheinlichkeit p erreicht ist. In der Regel wird nur Temperature verwendet.
    \item \textbf{Frequency/Presence Penalty} Bestrafen wiederholter bzw. neuer Tokens. In Produktion meist auf 0.0 gelassen.
    \item \textbf{Max Completion Tokens} Begrenzt die Länge der Modell-Antwort. Wichtigster Kostenhebel nach der Modell-Wahl.
    \item \textbf{Stop Sequences} Strings, bei deren Auftreten die Generierung abgebrochen wird. Nützlich für strukturierte Outputs.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Chat Completions API:} Der Kern -- Interaktion mit GPT-Modellen via Message-Passing (system/user/assistant). Unterstützt Text, Bilder und Audio als Input sowie strukturierte JSON-Ausgaben.
    \item \textbf{Responses API (neuer Standard):} OpenAIs neue generische Endpoint-Struktur, die Chat Completions ablöst. Führt Stream, Tool Calling, Parallel Tool Calling, Truncation und mehr unter einem einheitlichen Interface zusammen.
    \item \textbf{Fine-Tuning API:} Training von Modellen auf eigenen Daten, ohne das Basismodell selbst trainieren zu müssen.
    \item \textbf{Embeddings API:} Umwandlung von Text in Vektoren für Semantic Search und Similarity Matching.
    \item \textbf{Moderation API:} Klassifikation von Inhalten nach OpenAI-Safety-Richtlinien.
    \item \textbf{Batch API:} Asynchrone Verarbeitung großer Mengen von API-Calls mit 50\% Rabatt.
    \item \textbf{Realtime API:} WebSocket-basierte Echtzeit-Kommunikation für Audio/Text.
    \item \textbf{Assistants API:} Vorgefertigte Agenten-Infrastruktur (Knowledge Retrieval, Code Interpreter).
\end{itemize}
...
\praxishinweis{Die Rollen-Trennung (system/user/assistant/tool) ist kein Detail -- sie ist das Fundament für verlässliche Multi-Turn-Interaktionen und Tool-Use. System-Prompt definiert das Verhalten, User-Prompt die Aufgabe, assistant/tool die Interaktionsschleife. Vermische diese Rollen nicht.}
...
\praxishinweis{Structured Outputs sollten der Standard für jeden API-Call sein, der in einem Produktivsystem landet. Ohne Schema-Enforcement produziert jedes LLM gelegentlich invalides JSON -- und das verursacht Produktionsfehler, die schwer zu debuggen sind.}
...
\praxishinweis{Streaming ist für User-facing APIs Pflicht, aber für Backend-Processing oft hinderlich. Batch-API (50\% Rabatt) oder synchroner Modus sind für Massenverarbeitung besser geeignet. Wähle den Modus nach Use-Case, nicht nach Gewohnheit.}
...
\praxishinweis{Fine-Tuning wird oft überschätzt. Bevor du einen Fine-Tuning-Job startest, prüfe: 1) Reicht Prompt-Engineering + RAG? 2) Kann Prompt-Caching das Problem lösen? 3) Ist das Datenformat korrekt? Fine-Tuning ist mächtig, aber teuer und wartungsintensiv.}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Model-Version pinning}: Nutze datierte Versionen (\texttt{gpt-4o-202X-XX-XX}) statt generischer (\texttt{gpt-4o}).
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Audit-Trail}: Jeder API-Call wird protokolliert (User-ID, Prompt-Länge, Kosten)
\end{itemize}
