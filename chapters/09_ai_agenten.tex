...
Ein einfacher LLM-Aufruf ist reaktiv: Prompt rein, Antwort raus. Für Klassifikation, Extraktion, Zusammenfassung reicht das. Aber sobald ein System \textbf{mehrere Schritte} ausführen, \textbf{Tools aufrufen} und \textbf{Ergebnisse interpretieren} muss, reicht ein einzelner Prompt nicht.
...
\begin{praxishinweis}
Tool-Beschreibungen sind die halbe Miete. Ein schlecht beschriebenes Tool wird nie aufgerufen. Jede Tool-Definition braucht: präzisen Namen, klaren Einsatzzweck, dokumentierte Parameter mit Beispielwerten. Maximal 5--10 Tools gleichzeitig -- bei mehr: hierarchische Tool-Auswahl.
\end{praxishinweis}
...
\begin{praxishinweis}
Multi-Agent-Systeme multiplizieren Kosten und Latenz: 5 Agenten × 3 Schritte × 4 LLM-Calls = 60 Calls pro Task. Immer Budget-Limits setzen und Modell-Tiering nutzen (günstiges Modell für Routine, teures für Reasoning). Ein einzelner Prompt reicht oft -- Agent nur bei Multi-Step-Aufgaben mit Tool-Bedarf.
\end{praxishinweis}
...
\begin{praxishinweis}
Agenten sind mächtiger als einfache LLM-Calls -- und damit gefährlicher. Ein Agent ohne max\_steps, Token-Budget und Tool-Whitelist kann Tausende API-Calls auslösen (Denial-of-Wallet). Jedes Tool-Ergebnis ist ein potenzielles Injection-Vehikel. HITL für kritische Aktionen ist keine Option, sondern Pflicht.
\end{praxishinweis}
