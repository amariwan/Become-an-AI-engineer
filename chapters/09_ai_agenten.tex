...
\begin{enumerate}[leftmargin=*]
    \item \textbf{RAG vs AI Agent?}

    \textbf{Antwort:} RAG ist passiv (1 Prompt = 1 Antwort). Agents sind aktiv (Multi-Step-Loop: Planen, API ausführen, Evaluieren, Wiederholen).

    \item \textbf{ReAct-Loop Architektur?}

    \textbf{Antwort:} Thought (Überlegung) → Action (Tool Call) → Observation (API-Ergebnis parsen) → Repeat bis finale Antwort gefunden.

    \item \textbf{Endlosschleifen (Agent hängt fest) verhindern?}

    \textbf{Antwort:} Harter `max\_steps` Counter, Execution-Timeouts und Token-Budget-Tracking pro Session.

    \item \textbf{Parallel Tool Calling Vorteil?}

    \textbf{Antwort:} Reduziert Latenz und LLM-Calls um bis zu 50\%, da unabhängige Tools (z.B. Wetter in Berlin UND München) im selben Turn ausgeführt werden.

    \item \textbf{Human-in-the-Loop (HITL) bei kritischen Agent-Aktionen?}

    \textbf{Antwort:} Agent stoppt Execution, schiebt Status in eine `pending\_approvals` DB. Mensch bestätigt via UI, Agent nimmt State wieder auf.

    \item \textbf{Agent ruft ständig das falsche Tool auf. Fixes?}

    \textbf{Antwort:} Tool-Descriptions präzisieren (als Pydantic Docstrings), Few-Shot Beispiele für die Tool-Wahl einbauen, Anzahl der Tools reduzieren (Max 5-7 pro Agent).

    \item \textbf{Semantic Routing für Tools?}

    \textbf{Antwort:} Statt alle Tools ins Prompt zu laden, embeddet man die Query und lädt per RAG nur die 3 relevantesten Tool-Definitionen in den System-Prompt (Spart Kontext/Kosten).

    \item \textbf{Single-Agent vs Multi-Agent (z.B. Autogen/CrewAI)?}

    \textbf{Antwort:} Single-Agent für lineare Workflows. Multi-Agent für komplexe Domains, bei denen verschiedene Personas (Coder, Reviewer, QA) mit unterschiedlichen System-Prompts interagieren.

    \item \textbf{Wie behält der Agent Zustand (Memory) über lange Sessions?}

    \textbf{Antwort:} Sliding-Window Memory (letzte N Turns) plus Summary-Memory (LLM komprimiert alte Turns) plus Vector-Memory (RAG auf vergangene Sessions).

    \item \textbf{Angreifer übergibt dem Agenten über ein Tool bösartigen Code zur Ausführung. Härtung?}

    \textbf{Antwort:} Code-Execution-Tools streng in ephemeren, netzwerklosen Docker-Sandboxes (z.B. E2B) isolieren. Egress-Traffic hart blockieren.
