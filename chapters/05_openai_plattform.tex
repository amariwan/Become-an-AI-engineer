...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Structured Outputs (strict: true) lehnt Schema ab. Ursache?}

    \textbf{Antwort:} `strict: true` unterstützt kein vollumfängliches JSON Schema (keine optionalen Felder ohne `null`, keine Rekursion).

    \item \textbf{Batch API mit 50.000 Docs failed. Wie isolierst du den Fehler?}

    \textbf{Antwort:} Error-File-ID aus der Batch-Response laden, die die `custom\_ids` der gefehlten Requests enthält.

    \item \textbf{User schließt Tab bei Streaming nach 2s. Wie stoppst du LLM-Kosten?}

    \textbf{Antwort:} Verbindungsabbruch im Backend erkennen (z.B. FastAPI Request-Cancellation) und Stream per `response.close()` hart beenden.

    \item \textbf{Parallel Tool Calling: Reihenfolge der Ergebnisse?}

    \textbf{Antwort:} Reihenfolge ist irrelevant, solange die `tool\_call\_id` exakt gematcht wird und alle in der History stehen.

    \item \textbf{10K System-Prompt: Fine-Tuning auf gpt-4o-mini statt Prompting. Wirtschaftlich?}

    \textbf{Antwort:} Selten. Fine-Tuned Modelle kosten pro Token oft das 2-3fache. Mit Prompt Caching ist der lange System-Prompt meist günstiger.

    \item \textbf{App reicht User-Bilder an Vision API — Account nach 3 Tagen gesperrt. Architekturfehler?}

    \textbf{Antwort:} Fehlende Moderation. Bilder müssen vor dem LLM-Call durch die Moderations-API auf NSFW/Gewalt geprüft werden.

    \item \textbf{`temperature=0.0`, Output weicht trotzdem ab. Wie Determinismus erhöhen?}

    \textbf{Antwort:} `seed` Parameter setzen und den `system\_fingerprint` in der Response überwachen. (100\% Determinismus gibt es bei GPUs nie).

    \item \textbf{Prompt-Caching bei [User-Frage] + [RAG] + [System]. Fehler?}

    \textbf{Antwort:} Invalidiert Prefix sofort. Korrekt: [System] + [RAG] + [User-Frage].

    \item \textbf{`presence\_penalty=2.0` für Python-Code. Modell halluziniert. Warum?}

    \textbf{Antwort:} Penalty bestraft Wiederholungen. Bei Code sind Schlüsselwörter (def, return) essenziell. Für Code: Penalties auf 0 setzen.

    \item \textbf{400 Bad Request bei OpenAI → Fallback zu Anthropic triggern?}

    \textbf{Antwort:} Nein. 400 ist ein Client-Fehler (falsches Schema/Auth). Der Fallback würde auch fehlschlagen. Fallbacks nur bei 5xx oder 429.
