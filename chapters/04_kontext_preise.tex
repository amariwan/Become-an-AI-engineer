...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Erste drei Optimierungsmaßnahmen bei hohen API-Kosten?}

    \textbf{Antwort:} 1) Semantic Router (Mini für Standard). 2) Prompt Caching für statische Kontexte. 3) Strikte `max\_tokens` Limitierung zur DoS-Prävention.

    \item \textbf{RAG mit 50K Kontext im 128K Fenster. Antworten oft ungenau. Problem?}

    \textbf{Antwort:} Lost in the Middle. Lösung: Re-Ranker nutzen und den Kontext dynamisch auf die Top-K Tokens (z.B. 10K) eindampfen.

    \item \textbf{Monatskosten bei 100K Queries/Tag (GPT-4o-mini, 1K In, 500 Out)?}

    \textbf{Antwort:} (1.000 * \$0.15/1M + 500 * \$0.60/1M) * 100.000 * 30 = ca. \$1.350/Monat.

    \item \textbf{Wann lohnt sich Self-Hosting gegenüber APIs finanziell?}

    \textbf{Antwort:} Break-Even liegt meist ab ca. 50-100 Mio. Tokens/Tag, vorausgesetzt GPU-Abschreibung und MLOps-Personalkosten sind einkalkuliert.

    \item \textbf{Wie evaluierst du Routing-Entscheidungen (Small vs Large Model)?}

    \textbf{Antwort:} Golden Dataset mit 500 Queries. Metrik: Routing Accuracy (gehts ans richtige Modell?) + Cost-Impact (wieviel \$ gespart?).

    \item \textbf{Wie vermeidest du Context Window Overflows zuverlässig?}

    \textbf{Antwort:} Pre-Call-Token-Counting mit `tiktoken`. RAG-Chunks oder Chat-History dynamisch trimmen, bevor der API-Call abgesetzt wird.

    \item \textbf{Prompt-Caching zeigt 0 \% Hits, obwohl der System-Prompt gleich bleibt. Fehler?}

    \textbf{Antwort:} Die Reihenfolge. Dynamische Elemente (User-Input) stehen vor dem statischen System-Prompt. Prefix-Matching bricht sofort ab.

    \item \textbf{Modell A (\$1 In/\$1 Out) vs. Modell B (\$0.50 In/\$3 Out). Welches ist für RAG günstiger?}

    \textbf{Antwort:} Modell B. RAG ist stark asymmetrisch (Massiv Input-Tokens für Kontext, wenig Output-Tokens für Antwort).

    \item \textbf{Ein User sendet 100x ein 120K Token Dokument und fragt "Zusammenfassung?". Kosten explodieren. Prävention?}

    \textbf{Antwort:} Input-Token-Rate-Limiting pro User und Tag implementieren, nicht nur Request-Rate-Limiting.

    \item \textbf{Warum verbraucht ein "JSON Mode" Output mehr Tokens als ein reiner String?}

    \textbf{Antwort:} Wegen Whitespaces, Zeilenumbrüchen und Keys. Lösung: Minifiziertes JSON ohne Einrückungen anfordern.
