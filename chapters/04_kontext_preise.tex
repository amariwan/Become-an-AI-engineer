...
\bottomrule
\end{tabularx}
\caption{Kontextfenster der wichtigsten Modelle (2026)}
\end{table}
...
\subsection{Token-Kosten pro Millionen Tokens (2026)}
...
\begin{description}[leftmargin=*,style=nextline]
    ...
    Compression für lange Sessions. Der Standard 2026.
\end{description}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Ein Kunde beschwert sich über zu hohe LLM-Kosten.
    Was sind deine ersten drei Optimierungsmassnahmen?}

    \textbf{Antwort:} 1) Semantic Router: einfache Queries auf
    GPT-4o-mini routen. 2) Prompt-Caching für System-Prompt und
    Few-Shot-Beispiele. 3) Output-Tokens auf das notwendige Minimum
    begrenzen (\texttt{max\_tokens}). Diese drei Massnahmen sparen
    typischerweise 60--80\,\% der Kosten.

    \item \textbf{Dein RAG-System nutzt 50K Tokens Kontext pro Query.
    Das Modell hat 128K Fenster. Trotzdem sind die Antworten schlecht.
    Was ist das Problem?}

    \textbf{Antwort:} Lost in the Middle. Die relevanten Dokumente
    sind in der Mitte des Kontexts begraben. Lösung: Re-Ranking, um
    die top-3 relevantesten Dokumente an den Anfang des Kontexts zu
    setzen. Ausserdem: Weniger Kontext (10K statt 50K), dafür
    relevanter.

    \item \textbf{Wie berechnest du die monatlichen Kosten einer
    LLM-Anwendung mit 100.000 täglichen Queries?}

    \textbf{Antwort:} Formel: (Input-Tokens/Query $\times$ Input-Preis
    + Output-Tokens/Query $\times$ Output-Preis) $\times$ Queries/Tag
    $\times$ 30 Tage. Für ein RAG-System mit GPT-4o-mini (8K Input,
    500 Output): (8.000 $\times$ 0,15 \$/M + 500 $\times$ 0,60 \$/M)
    $\times$ 100.000 $\times$ 30 = ca. 4.500 \$/Monat.

    \item \textbf{Wann lohnt sich Self-Hosting statt API?}

    \textbf{Antwort:} Ab etwa 50 Mio. Tokens/Tag. Bei geringerem
    Volumen ist API günstiger (keine GPU-Kosten, keine Operations).
    Bei sehr hohem Volumen (500 Mio. Tokens/Tag) kann Self-Hosting
    5--10x günstiger sein.

    \item \textbf{Dein Team nutzt ein Semantic Router-System.
    Wie evaluierst du, ob die Routing-Entscheidungen korrekt sind?}

    \textbf{Antwort:} Golden Dataset mit 500 Queries, gelabelt mit
    der optimalen Modell-Kategorie. Metrik: Routing Accuracy
    (Prozentsatz der korrekt gerouteten Queries). Zusätzlich
    Kosten-Impact-Metrik: Wie viel Geld spart der Router im
    Vergleich zu immer GPT-4o?

    \item \textbf{Wie stellst du sicher, dass du nie das Context
    Window überschreitest?}

    \textbf{Antwort:} Pre-Call-Token-Counting mit tiktoken, bevor der
    API-Call rausgeht. Wenn der Prompt zu lang ist: Chat-History
    kürzen (Sliding Window) oder RAG-Kontext reduzieren. Niemals
    einen Prompt ohne Token-Count senden.

    \item \textbf{Du lädst 100 Dokumente (insgesamt 50K Tokens) in
    einen Prompt. Das Modell ignoriert wichtige Details aus Dokument
    47. Was ist passiert und wie behebst du es?}

    \textbf{Antwort:} Lost in the Middle -- Dokument 47 liegt genau
    in der Mitte des Kontexts und bekommt weniger Attention.
    Lösung: RAG mit Re-Ranking. Dokumente nach Relevanz sortieren,
    sodass die wichtigsten an erster Stelle stehen. Maximal 5--10
    Dokumente in den Prompt geben.

    \item \textbf{Du setzt Prompt-Caching ein, aber das Dashboard
    zeigt 0\,\% Cache Hits. Was ist falsch?}

    \textbf{Antwort:} Caching funktioniert nur bei exakt identischen
    Präfixen. Typische Fehler: 1) Dynamische Werte im System-Prompt
    (Timestamp, User-Name). 2) Unterschiedliche Reihenfolge der
    Nachrichten. 3) Modell-String ändert sich (gpt-4o vs gpt-4o-2026-01).
    Lösung: Cache-Key auf statischen Teil reduzieren.
\end{enumerate}
