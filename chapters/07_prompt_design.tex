...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Zero-Shot vs Few-Shot in Produktion?}

    \textbf{Antwort:} Zero-Shot ist günstiger. Few-Shot (3–5 Beispiele) erhöht die Zuverlässigkeit um 20-30\%, kostet aber mehr Input-Tokens.

    \item \textbf{Wann nutzt du Chain-of-Thought (CoT)?}

    \textbf{Antwort:} Bei logischen, mathematischen oder mehrstufigen analytischen Aufgaben. Zwingt das LLM, Zwischenschritte im KV-Cache auszurollen.

    \item \textbf{Schutz vor Prompt-Injection in Produktion?}

    \textbf{Antwort:} User-Input in `<user\_input>` XML-Tags isolieren, Input-Sanitization und Output über DLP-Scanner prüfen.

    \item \textbf{Self-Consistency Pattern?}

    \textbf{Antwort:} Das LLM generiert parallel 5 Antworten via Temperatur > 0. Eine Majority-Vote Logik wählt das finale Ergebnis.

    \item \textbf{Wie versionierst und verwaltest du Prompts im Team?}

    \textbf{Antwort:} Als Code. `.jinja2` Templates im Git-Repo, deployt in eine Model-Registry oder Feature-Store, validiert über CI/CD-Evals.

    \item \textbf{ReAct vs einfaches Tool Calling?}

    \textbf{Antwort:} Tool Calling führt exakt ein Tool aus. ReAct evaluiert das Ergebnis in einer internen Schleife und entscheidet dynamisch über weitere Schritte.

    \item \textbf{Was ist "Negative Prompting" und wann versagt es?}

    \textbf{Antwort:} Anweisungen wie "Verwende keine Metaphern". Versagt oft bei LLMs, da das Konzept (Metapher) dadurch in der Attention-Matrix aktiviert wird. Besser: Positiv formulieren ("Schreibe extrem sachlich").

    \item \textbf{Du hast 50 Few-Shot Beispiele. Alle reinladen?}

    \textbf{Antwort:} Nein (Token-Verschwendung und Lost-in-the-middle). Dynamisches Few-Shotting: Die User-Query embedden und via Vector-Search nur die 3 relevantesten Beispiele in den Prompt laden.

    \item \textbf{Chain-of-Verification Architektur?}

    \textbf{Antwort:} LLM 1 generiert Baseline-Antwort. LLM 2 generiert Verifizierungsfragen dazu. LLM 3 beantwortet diese unabhängig. LLM 4 schreibt finale, korrigierte Antwort.

    \item \textbf{Prompt-Qualität evaluieren nach Anpassungen?}

    \textbf{Antwort:} Über ein Golden Dataset (Ground Truth). Automatischer CI-Run misst Accuracy, Precision und Token-Effizienz (LLM-as-a-Judge).
