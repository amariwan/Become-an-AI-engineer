...
Prompt Engineering ist die \textbf{wichtigste Programmierschnittstelle} zwischen Mensch und Maschine. Anders als klassisches Software Engineering, wo Code deterministisch ausgeführt wird, interagiert Prompt Engineering mit einem probabilistischen System. Du schreibst Code, der von einem trainierten Modell interpretiert wird -- nicht deterministisch, sondern als Wahrscheinlichkeitsverteilung.

Derselbe Prompt liefert nicht immer dieselbe Antwort. Ein Unterschied in der Formulierung ("klassifiziere" vs. "kategorisiere") kann Ergebnisse verändern. Ohne systematisches Prompt Engineering produzieren LLM-Integrationen inkonsistente Ergebnisse.

Unternehmen mit \textbf{strukturiertem Prompt Engineering} haben 3--5x niedrigere Fehlerraten und 40--60\,\% niedrigere Token-Kosten. Prompt Engineering ist kein "Schreiben einer guten Frage" -- es ist eine systematische Engineering-Disziplin.
...
Prompt Engineering formuliert Eingabe-Anweisungen (Prompts), damit ein LLM konsistent, verlässlich und effizient die gewünschte Ausgabe liefert.

Software-Entwickler:innen schreiben Code, um Maschinen zu instruieren. AI Engineer:innen schreiben Prompts, um KI-Modelle zu instruieren. Beide definieren die Logik eines Systems. Der Unterschied: Code ist deterministisch, ein Prompt ist eine Wahrscheinlichkeitsverteilung über Token-Sequenzen.

\subsection{Die drei Ebenen eines Prompts}

\begin{praxishinweis}
Jeder Prompt hat drei Ebenen: Instruktion (was tun), Kontext (worum geht es), Metaebene (wie denken). Fehlt eine, rät das Modell. Explizite Metaebene -- Persona, Ton, Format -- ist der einfachste Hebel für konsistente Ergebnisse.
\end{praxishinweis}
...
\begin{praxishinweis}
Faustregel: Instruktionen so kurz wie möglich, Beispiele so viele wie nötig, Kontext so wenig wie möglich. Messen statt raten -- \texttt{prompt\_efficiency\_score()} im Performance-Abschnitt hilft, die optimale Länge zu finden.
\end{praxishinweis}
...
\begin{praxishinweis}
Prompt-Länge ist nicht nur Qualität, sondern auch Kosten. Ein Prompt mit 4.000 Tokens kostet 4x mehr als einer mit 1.000 Tokens -- bei oft nur marginalem Qualitätsgewinn. Miss die \texttt{prompt\_efficiency\_score()} und optimiere auf Tokens pro Qualitätspunkt.
\end{praxishinweis}
...
\begin{praxishinweis}
Prompt-Injection ist kein theoretisches Problem. Jeder User-Input kann schädliche Anweisungen enthalten. Die goldene Regel: User-Input niemals in den System-Prompt interpolieren. XML-Tags, Rollen-Isolation und Output-Filter sind die Minimal-Absicherung für Produktion.
\end{praxishinweis}
