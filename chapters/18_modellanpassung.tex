...
\begin{enumerate}[leftmargin=*]
    \item \textbf{LoRA-Mechanismus. Was ist der Rank (\$r\$)?}

    \textbf{Antwort:} Statt Milliarden Gewichte anzupassen, lernt LoRA zwei kleine Low-Rank-Matrizen \$A\$ und \$B\$. Der Rank \$r\$ definiert die "Lernkapazität" (Dimension). Zu hoch = Overfitting, zu niedrig = Underfitting.

    \item \textbf{Multi-LoRA Serving in Produktion?}

    \textbf{Antwort:} Das schwere Basismodell (z.B. Llama 3) bleibt 1x im VRAM. Die winzigen LoRA-Adapter (wenige MB) werden per Request (Dynamic Adapter Loading) im vLLM on-the-fly in den Batch injiziert.

    \item \textbf{Modell verlernt nach Fine-Tuning sein Allgemeinwissen. Fix?}

    \textbf{Antwort:} Catastrophic Forgetting. Das Dataset muss Replay-Data (20-30\% alte, generische Prompts) enthalten, um die alten Gewichtungs-Cluster zu stabilisieren.

    \item \textbf{DPO (Direct Preference Optimization) vs RLHF?}

    \textbf{Antwort:} RLHF erfordert ein separates Reward-Model und komplexe PPO-Pipelines. DPO optimiert das Modell direkt mathematisch auf Basis von "Chosen vs. Rejected" Paaren. DPO ist leichter, billiger und stabiler.

    \item \textbf{Wie bestimmst du die Lernrate für LoRA vs Full Fine-Tuning?}

    \textbf{Antwort:} LoRA benötigt viel höhere Lernraten (z.B. `2e-4` bis `1e-3`), da von Null trainiert wird. Full Fine-Tuning nutzt minimale Raten (`1e-5`), da vortrainierte Strukturen zerstört werden könnten.

    \item \textbf{Wann nutzt man Prefix-Tuning anstelle von LoRA?}

    \textbf{Antwort:} Wenn man das Modellverhalten nur temporär für spezifische Tasks "lenken" will, ohne die Modellgewichte anzufassen (Virtuelle Tokens werden in den Attention-Head injiziert).

    \item \textbf{Minimale Dataset-Größe für Format-Fine-Tuning (z.B. Custom JSON)?}

    \textbf{Antwort:} Für reine Syntax-Imitation reichen 100-300 hochqualitative, diverse Beispiele. Quantität ist irrelevant, Data-Quality (keine Fehler im Target-Output) ist alles.

    \item \textbf{Evaluierung während des Fine-Tunings?}

    \textbf{Antwort:} Validation Set (20\% der Daten) nutzen. Den Validation-Loss überwachen. Steigt der Val-Loss während der Training-Loss sinkt, fängt das Modell an zu memorisieren (Overfitting) → Early Stopping.

    \item \textbf{Merging: LoRA-Adapter dauerhaft ins Basismodell integrieren?}

    \textbf{Antwort:} Via Weight-Merging Scripts (z.B. PEFT). Die $A \times B$ Matrix wird mathematisch auf die Basis-Gewichte addiert. Eliminiert den minimalen Inference-Overhead von LoRA.

    \item \textbf{Hardware-Setup für 8B Modell Fine-Tuning (QLoRA)?}

    \textbf{Antwort:} Basismodell in 4-Bit laden. Training auf einer Single-GPU mit 16-24GB VRAM (z.B. RTX 4090). Batch-Size 1-2 mit Gradient Accumulation, um VRAM-OOM zu verhindern.
