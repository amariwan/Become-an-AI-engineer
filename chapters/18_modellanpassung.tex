...
\begin{praxishinweis}
Die beste Architektur kombiniert alle drei: Prompt Engineering für den System-Prompt, RAG für aktuelle Fakten, Fine-Tuning für Format und Stil. Nicht entweder-oder, sondern alle drei geschichtet.
\end{praxishinweis}
...
\begin{praxishinweis}
QLoRA ist der Game-Changer für Teams ohne GPU-Cluster. 70B-Modell auf einer A80 fine-tunen -- aber aufgepasst: Das Basismodell in 4-Bit bedeutet leichten Qualitätsverlust. Für maximale Quality lieber 8B in FP16 mit LoRA.
\end{praxishinweis}
...
\begin{praxishinweis}
500 qualitätsgeprüfte Beispiele schlagen 5.000 synthetische. Jeder Fehler im Dataset wird vom Modell gelernt. Manuelle Prüfung von 20\% aller Einträge ist Pflicht -- besonders bei synthetisch generierten Daten.
\end{praxishinweis}
...
\begin{praxishinweis}
Forgetting-Check ist kein Nice-to-have. Ein FT-Modell, das auf SQL glänzt aber "Was ist 2+2?" nicht mehr beantwortet, ist unbrauchbar. Fix: 20-30\% generische Prompts im Dataset behalten.
\end{praxishinweis}
...
\begin{praxishinweis}
Multi-LoRA-Serving ist der effizienteste Weg, mehrere spezialisierte Modelle zu betreiben. Das Basismodell bleibt 1x im VRAM, Dutzende Adapter teilen sich den Rest. Ideal für SQL-, E-Mail- und Code-Agents auf einer GPU.
\end{praxishinweis}
