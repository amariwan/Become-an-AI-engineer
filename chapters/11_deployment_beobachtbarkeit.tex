...
\begin{itemize}
    \item Eine LLM-Anwendung als FastAPI-Microservice deployen
    \item Latenz-Optimierungstechniken wie Streaming, Caching und Modell-Tiering anwenden
    \item Fehler-Toleranz mit Retry, Fallback und Circuit-Breaker implementieren
    \item Observability mit Metriken, Logging und Tracing aufbauen
    \item Ein Deployment mit Docker und CI/CD automatisieren
\end{itemize}
\end{lernziele}
...
Eine LLM-Anwendung in Produktion zu bringen unterscheidet sich grundlegend vom klassischen Software-Deployment. In einer traditionellen Web-App ist das Verhalten deterministisch: Gleicher Input, gleicher Output. Bei LLMs ist das nicht der Fall -- und das hat weitreichende Konsequenzen.

Drei Herausforderungen machen LLM-Deployment einzigartig:

\begin{enumerate}[leftmargin=*]
    \item \textbf{External Dependencies}: Deine App hängt von der API-Verfügbarkeit ab (OpenAI, Anthropic). Wenn die API ausfällt, fällt deine App mit aus. Du hast keine direkte Kontrolle über die Latenz oder Verfügbarkeit.
    \item \textbf{Kosten-Kontrolle}: Jeder Request kostet Geld. Ohne Monitoring explodieren die Kosten bei Popularität. Ein unerwarteter Traffic-Spike kann innerhalb von Stunden tausende Euro kosten.
    \item \textbf{Probabilistisches Verhalten}: Dein Test kann heute bestehen und morgen trotzdem falsche Antworten liefern. Deterministische Unit-Tests allein reichen nicht -- du brauchst kontinuierliche Evaluation in der Produktion.
\end{enumerate}
...
Es gibt vier grundlegende Deployment-Modelle für LLM-Anwendungen:

\begin{description}[leftmargin=*,style=nextline]
    \item[API-basiert (Managed)] Du rufst eine externe API auf (OpenAI, Anthropic, Google). Kein Modell-Hosting notwendig. Pay-per-Token. Einfachster Einstieg, aber Vendor-Lock-in und Latenz-abhängig.
    \item[Self-hosted Open Source] Du hostest ein Open-Source-Modell (Llama, Mistral, Qwen) auf eigener Infrastruktur. Volle Kontrolle, keine Datenverlassen den Server. Aber hohe Infrastruktur-Kosten (GPUs).
    \item[Managed Inference (Cloud)] Du nutzt einen Inference-Anbieter (Together AI, Fireworks, Groq), der Open-Source-Modelle als API anbietet. Kompromiss aus Kontrolle und Einfachheit.
    \item[Edge Deployment] Das Modell läuft auf dem Endgerät (Smartphone, Laptop). Minimale Latenz, keine Internetverbindung nötig. Aber stark limitierte Modellgrösse.
\end{description}
...
\begin{description}[leftmargin=*,style=nextline]
    \item[$T_{network}$] Netzwerk-Latenz zwischen Client und API. Bei Cloud-APIs typisch 50--200ms. Standort-Wahl (us-east vs. eu-west) macht bis zu 100ms Unterschied.
    \item[$T_{queue}$] Wartezeit auf dem Server. Bei hoher Auslastung der API können Requests Minuten in der Queue hängen. Rate Limiting verstärkt diesen Effekt.
    \item[$T_{prompt}$] Zeit für die Verarbeitung des Inputs (Pre-fill). Skaliert linear mit der Prompt-Länge. Ein 10K-Token-Prompt dauert ca. 5--15x länger als ein 1K-Token-Prompt.
    \item[$T_{generation}$] Zeit für die Generierung des Outputs. Skaliert linear mit der Anzahl Output-Tokens und quadratisch mit der Kontextlänge (Attention). Streaming reduziert die Wahrnehmung, nicht die tatsächliche Dauer.
    \item[$T_{post}$] Post-Processing: Output-Parsing, Safety-Checks, Logging. Typisch < 10ms.
\end{description}
...
\bottomrule
++ starke Verbesserung, + leichte Verbesserung, = neutral, -- Verschlechterung
...
Bei wiederholten API-Fehlern verhindert ein Circuit-Breaker weitere Versuche und gibt stattdessen eine Fallback-Antwort:
...
\begin{description}[leftmargin=*,style=nextline]
    \item[Metriken] Numerische Messwerte über Zeit: Requests/Minute, P95-Latenz, Cost/Query, Error Rate. Instrumentiere jeden Request mit Prometheus-kompatiblen Metriken.
    \item[Logging] Strukturierte Ereignis-Aufzeichnung: Jeder LLM-Call mit Prompt, Response, Latenz, Kosten. Verwende strukturiertes JSON-Logging für maschinenlesbare Logs.
    \item[Tracing] Request-Kontext über verteilte Systeme hinweg: Ein User-Request durchläuft API-Gateway, LLM-Service, Vector-DB, Cache. Tracing verbindet diese Spans zu einem Gesamtbild.
\end{description}
...
\begin{description}[leftmargin=*,style=nextline]
    \item[Kein Timeout gesetzt] Ohne Timeout blockiert dein Request ewig und frisst Resources. Ein LLM-Request kann bei Überlast Minuten dauern. Immer 10--30s Timeout.
    \item[API-Ausfall ignoriert] Wenn OpenAI down ist, sollte deine App eine Fallback-Nachricht zeigen statt einem 500er Error. Graceful Degradation ist kein Nice-to-have.
    \item[Caching vergessen] Ein Redis-Cache ist kein Micro-Optimization, sondern ein fundamentales Architektur-Element. Ohne Cache zahlst du für jede identische Query neu.
    \item[Monolith ohne Trennung] LLM-Logik, Business-Logik und Datenbank-Zugriff in einer Datei. Das skaliert nicht. Trenne in Service-Layer.
    \item[Hardcodierte Prompt-Templates] Prompt-Templates im Source-Code bedeuten ein Re-Deploy für jede Text-Änderung. Nutze Config-Dateien oder eine Datenbank.
    \item[Fehlende Kosten-Alarme] Ohne Budget-Alarme kann ein einziger Bug oder ein Traffic-Spike tausende Euro verbrauchen. Setze Schwellwerte und Alarmierung.
\end{description}
...
async def readiness():
    ...
    try:
        async with httpx.AsyncClient(timeout=5) as client:
            # Leichter Test-Call, minimale Kosten
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{LLM-Anwendung für 10.000 User deployen. Architektur?}

    \textbf{Antwort:} Stateless Container via Kubernetes (EKS). API-Gateway (Apisix) für Rate Limiting. LiteLLM als Backend-Proxy für Provider-Load-Balancing und Async-Logging via ClickHouse.

    \item \textbf{Vertikale vs Horizontale Skalierung bei LLMs?}

    \textbf{Antwort:} Vertikal (mehr VRAM) ist zwingend, um das Modell überhaupt in den Speicher zu laden. Horizontal (mehr Nodes) fängt den RPS-Durchsatz (Concurrency) ab.

    \item \textbf{Latenz steigt nach Deployment 5x. Erste Debug-Schritte?}

    \textbf{Antwort:} Distributed Tracing (OpenTelemetry) prüfen. Bottlenecks: KV-Cache-Eviction durch lange Prompts, DB-I/O-Latenz oder endlos generierende Tokens wegen fehlendem Max-Limit.

    \item \textbf{Readiness-Probes für eine LLM-API?}

    \textbf{Antwort:} Health-Endpoint pingt Vector-DB *und* macht einen 1-Token-Generierungstest ("Antworte OK"). Bei Fail wird der Pod vom Traffic isoliert.

    \item \textbf{Wie designst du Multi-Region Failover für RAG?}

    \textbf{Antwort:} Active-Active Setup hinter Global Load Balancer. Vektordatenbank asynchron replizieren. Session-State (Chat History) in globalem Redis/DynamoDB halten.

    \item \textbf{Streaming Load Balancer Problem?}

    \textbf{Antwort:} SSE-Verbindungen (Server-Sent Events) sind long-lived. Standard-Load-Balancer verteilen schlecht. Lösung: Layer-7 Balancing mit Least-Connections Algorithmus.

    \item \textbf{Cold Start bei Serverless LLMs (z.B. AWS SageMaker Serverless)?}

    \textbf{Antwort:} Das Laden von 150GB Gewichten in den VRAM dauert Minuten. Serverless ist für synchrone LLMs tot. Provisioned Concurrency (immer laufende GPUs) ist zwingend.

    \item \textbf{Tracking von Token-Limits pro Tenant in Echtzeit?}

    \textbf{Antwort:} Token-Bucket-Algorithmus in Redis. Nach jedem API-Call wird der zurückgelieferte Token-Verbrauch asynchron vom Mandanten-Bucket subtrahiert.

    \item \textbf{Semantic Caching Deployment?}

    \textbf{Antwort:} Dedizierter Microservice *vor* dem LLM-Gateway. Nutzt schnelles lokales Embedding-Modell und Vektor-Suche in Redis (RedisVL) mit 95\% Confidence-Threshold.

    \item \textbf{Welche 3 Metriken im Dashboard sind nachts um 3 Uhr Pager-würdig?}

    \textbf{Antwort:} 1) Error Rate der Provider-API > 5\%. 2) Time-to-First-Token (TTFT) P95 > 2 Sekunden. 3) Token-Budget Spend-Rate Threshold überschritten.
...
\textbf{Nächster Schritt:} Im nächsten Kapitel geht es um AI Safety und Ethics -- Themen die in der Praxis oft unterschätzt werden, aber kritisch sind für jeden produktiven KI-Einsatz.
