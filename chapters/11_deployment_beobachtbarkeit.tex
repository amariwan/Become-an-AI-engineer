...
\begin{praxishinweis}
External Dependencies sind kein reines Availability-Problem. Ein API-Provider kann verfügbar sein, aber plötzlich 10x höhere Latenz liefern. Baue Timeout-Monitoring und Latenz-Alarme ein -- nicht nur Health-Checks.
\end{praxishinweis}
...
\begin{description}[leftmargin=*,style=nextline]
    \item[API-basiert (Managed)] Externe API (OpenAI, Anthropic, Google). Kein Modell-Hosting. Pay-per-Token. Einfachster Einstieg, aber Vendor-Lock-in und Latenz-abhängig.
    \item[Self-hosted Open Source] Open-Source-Modell (Llama, Mistral, Qwen) auf eigener Infrastruktur. Volle Kontrolle, keine Daten verlassen den Server. Hohe Infrastruktur-Kosten (GPUs).
    \item[Managed Inference (Cloud)] Inference-Anbieter (Together AI, Fireworks, Groq) bietet Open-Source-Modelle als API. Kompromiss aus Kontrolle und Einfachheit.
    \item[Edge Deployment] Modell läuft auf Endgerät (Smartphone, Laptop). Minimale Latenz, keine Internetverbindung nötig. Stark limitierte Modellgrösse.
\end{description}
...
\begin{praxishinweis}
API Gateway und LLM-Service sollten getrennte Deployments sein. Ein Rate-Limit-Treffer soll nicht den gesamten Service blockieren, sondern nur den entsprechenden Client. Trennung erlaubt unabhängiges Skalieren.
\end{praxishinweis}
...
\begin{description}[leftmargin=*,style=nextline]
    ...
    \item[$T_{queue}$] Wartezeit auf dem Server. Bei hoher Auslastung können Requests Minuten in der Queue hängen. Rate Limiting verstärkt diesen Effekt.
    ...
    \item[$T_{generation}$] Zeit für die Generierung des Outputs. Skaliert linear mit Output-Tokens und quadratisch mit der Kontextlänge (Attention). Streaming reduziert die Wahrnehmung, nicht die Dauer.
...
\begin{praxishinweis}
Streaming verbessert die subjektive Latenz massiv, reduziert aber nicht die Gesamtverarbeitungszeit. Der erste Token kommt schneller, die komplette Antwort braucht gleich lang. Für UX zählt Time-to-First-Token mehr als Total-Time.
\end{praxishinweis}
...
\begin{praxishinweis}
Kubernetes-Proben unterscheiden: livenessProbe = "Pod neu starten", readinessProbe = "Traffic stoppen". Ein LLM-Service mit Timeout sollte readiness-failen (Traffic umleiten), nicht liveness-failen (was einen Restart auslöst der Minuten dauert).
\end{praxishinweis}
