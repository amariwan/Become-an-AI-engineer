...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Junior will 8x7B MoE-Modell auf 16-GB-GPU hosten. Denkfehler?}

    \textbf{Antwort:} Alle Gewichte (ca. 47B) müssen im VRAM liegen für das Routing, nicht nur die pro Token aktivierten Parameter.

    \item \textbf{Semantic Router (Mini -> Opus) führt zu 2 Sekunden TTFT. Bottleneck?}

    \textbf{Antwort:} Sequenzielle Latenz. LLM-Calls in Reihe summieren sich. Lösung: Klassifikation durch lokale Modelle (FastText/BERT) mit 10ms Latenz ersetzen.

    \item \textbf{Neues OS-Modell schlägt GPT-4o, versagt aber bei striktem JSON. Warum?}

    \textbf{Antwort:} Mangelndes Alignment/Instruction Tuning für Formatvorgaben. Benchmarks testen oft nur Multiple-Choice.

    \item \textbf{Fallback feuert bei 429 sofort gegen Anthropic. Beide Provider offline. Warum?}

    \textbf{Antwort:} Thundering Herd Problem. Bei Ausfall wechseln alle User synchron zu B. Lösung: Exponential Backoff \& Jitter.

    \item \textbf{CFO will Chatbot auf DeepSeek R1 umstellen für 90\% Kostensenkung. Veto-Grund?}

    \textbf{Antwort:} Reasoning-Modelle (R1/o1) haben 5–15 Sekunden interne Chain-of-Thought vor dem ersten Output-Token. Tödlich für Echtzeit-Chat-UX.

    \item \textbf{Was ist die einzige Zero-Trust-Architektur für LLMs?}

    \textbf{Antwort:} Self-Hosting von Open-Weight-Modellen auf Air-Gapped On-Premise-Infrastruktur.

    \item \textbf{Abstraktion: Feature braucht OpenAIs json\_schema, Anthropic bietet das nicht. Lösung?}

    \textbf{Antwort:} Mapping auf Native Tool Calling (Function Calling) bei Anthropic, was das JSON-Format über Tool-Argumente erzwingt.

    \item \textbf{70B auf INT4: Code hat Syntax-Fehler, Smalltalk geht. Warum?}

    \textbf{Antwort:} Quantisierung zerstört feingranulare Gewichte. Code-Syntax ist hochsensibel, Smalltalk-Wahrscheinlichkeiten sind robuster.

    \item \textbf{System-Prompt 1:1 von GPT-4o auf Claude 3.5 portiert. Claude weigert sich. Warum?}

    \textbf{Antwort:} Unterschiedliches Alignment (Constitutional AI). Prompts sind nicht 1:1 portabel und müssen modell-spezifisch kalibriert werden.

    \item \textbf{10.000 PDF-Verträge am Tag analysieren, System-Prompt immer gleich. Kosten-Hack?}

    \textbf{Antwort:} Prompt Caching. Der statische System-Prompt muss exakt am Anfang (Prefix) des Arrays stehen, damit der Cache-Hit erfolgt.
