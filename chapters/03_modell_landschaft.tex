...
\begin{merke}
\textbf{Modellnamen im Wandel:} Dieses Kapitel nennt konkrete Modellnamen (GPT-4o, Claude~4, Gemini~2.5) als Beispiele zum Zeitpunkt der Drucklegung. Die Modell-Landschaft verändert sich rasant -- neue Versionen erscheinen monatlich. Die grundlegenden Konzepte der Modellklassen (Flaggschiff, effizient, spezialisiert) und Architekturentscheidungen bleiben davon unberührt. Prüfe die aktuellen Benchmark-Tabellen und Preislisten der Anbieter, bevor du eine Architekturentscheidung triffst.
\end{merke}
...
Es gibt nicht \glqq das eine beste Modell\grqq{}. Dutzende Modelle unterscheiden sich in Preis, Leistung, Latenz, Kontextfenster und Sicherheit. Die Wahl des falschen Modells kostet Unternehmen täglich tausende Euro -- durch überhöhte API-Kosten, schlechte Ergebnisse oder Ausfallrisiken (Single-Provider-Abhängigkeit).
...
\begin{description}[leftmargin=*,style=nextline]
    ...
    \item[Context Window] Die maximale Token-Anzahl, die ein Modell in einem Durchlauf verarbeiten kann. Zwischen 8K (klein) und 2M (Experimental).
    \item[Knowledge Cutoff] Das Datum, bis zu dem das Modell trainiert wurde. Modelle kennen keine Ereignisse nach ihrem Cutoff.
...
\begin{praxishinweis}
Bevor du Modelle vergleichst, kenne deine Anforderungen: Brauchst du Spitzenqualität, minimale Kosten, niedrige Latenz oder Datenhoheit? Keine Benchmark-Tabelle der Welt kann diese Frage beantworten -- nur dein eigener Anwendungsfall.
\end{praxishinweis}
...
\begin{praxishinweis}
Kein Closed-Source-Modell ist universell überlegen. GPT-4o glänzt bei Geschwindigkeit und Tool-Use, Claude bei Safety und langen Kontexten, Gemini bei Multimodalität. Teste alle drei auf deinen Daten, bevor du dich festlegst.
\end{praxishinweis}
...
\begin{praxishinweis}
Open-Source-Modelle wie Llama oder Mistral sind eine ernsthafte Alternative, nicht nur eine Notlösung. Rechne durch: Ab ca. 1 Mio. API-Anfragen/Monat kann Self-Hosting günstiger sein -- bei voller Datenkontrolle.
\end{praxishinweis}
...
\begin{praxishinweis}
Multi-Provider ist kein Luxus, sondern betriebliche Notwendigkeit. Ein einziger Provider bedeutet Single Point of Failure -- inklusive Preisrisiko. Drei Provider mit automatischem Fallback sind der Mindeststandard für Produktion.
\end{praxishinweis}
...
\begin{praxishinweis}
Die Entscheidungsmatrix ist ein Ausgangspunkt, nicht das letzte Wort. Jedes Team sollte ein eigenes Golden Dataset mit 100--500 repräsentativen Queries aufbauen und Modelle regelmäßig dagegen testen. Preise ändern sich, neue Modelle kommen -- deine Evaluierung muss mitwachsen.
\end{praxishinweis}
