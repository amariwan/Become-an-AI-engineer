\chapter{Die Model-Landschaft -- Ein Vergleich}
...
% ============================================================
\autornotiz{
2023 habe ich für ein Fintech ein Multi-Provider-Routing gebaut. 50k Requests/Tag.
OpenAI-Ausfall -> transparentes Failover auf Anthropic. User hat nichts gemerkt.
Die Lektion: Niemals auf einen Provider setzen. Abstraktions-Layer kostet 1 Tag Dev-Zeit,
spart Wochen bei Outage. Preise ändern sich monatlich. Wer nicht regelmäßig vergleicht,
zahlt drauf.
}
...
\skipbox{
\textbf{TL;DR:} Es gibt nicht \glqq das eine beste Modell\grqq{}. Dutzende Modelle unterscheiden sich in Preis, Leistung, Latenz, Kontextfenster und Sicherheit. Die Wahl des falschen Modells kostet Unternehmen täglich tausende Euro -- durch überhöhte API-Kosten, schlechte Ergebnisse oder Ausfallrisiken (Single-Provider-Abhängigkeit).
}

Ein AI Engineer muss die Landschaft nicht nur kennen, sondern aktiv steuern können: Wann setzt man GPT-4o-mini ein, wann Llama 4 lokal, wann DeepSeek R1 für Reasoning? Welche Modelle tauscht man aus, wenn der Preis steigt? Welcher Provider bietet EU-Datenhaltung?

Dieses Kapitel gibt einen strukturierten Überblick über den Modell-Markt, eine Entscheidungsmatrix und die Werkzeuge, um Modell-Entscheidungen systematisch zu treffen.

% ============================================================
% DIE ARCHITEKTUR DES MODERNEN AI-STACKS
% ============================================================
...
\begin{verbatim}
    ...
    |                                                            |
    |  1. LLM (Thinker)        Reasoning, Wissen, Kreativität    |
    |       |                                                    |
    |  2. RAG (Knowledge)      Kontext, Doku, Datenbanken        |
    |       |                                                    |
    |  3. Agents (Workers)     Ausführung, Automatisierung       |
    |       |                                                    |
...
RAG ist das Fundament für faktentreue Antworten. Kapitel~\ref{chap:rag_vector_datenbanken} behandelt RAG-Systeme ausführlich.

\subsection{Layer 3: Agents als Arbeiter}

Ein Agent (Kapitel~\ref{chap:ai_agenten}) hört nicht bei Textgenerierung auf. Er führt Aktionen aus: Tickets lesen, E-Mails senden, Berichte erstellen, CRMs aktualisieren, Code deployen, Workflows steuern. Agents verwandeln Intelligenz in Ausführung.
...
Die folgenden Abschnitte dieses Kapitels helfen dir, die richtige \textbf{erste Schicht} -- das LLM -- für deinen Stack zu wählen. Die übrigen Schichten werden in den Kapiteln~\ref{chap:rag_vector_datenbanken},~\ref{chap:ai_agenten} und im MCP-Exkurs vertieft.

% ============================================================
% GRUNDLAGEN -- MODELL-KLASSIFIKATION VERSTEHEN
% ============================================================
...
\begin{itemize}
    ...
    \item \textbf{Quantisierung} Reduktion der Parameter-Genauigkeit von FP16 auf INT8 oder FP4. Reduziert Speicherbedarf um 50--75\% bei geringem Qualitätsverlust. Unverzichtbar für Self-Hosting.
    \item \textbf{Context Window} Die maximale Token-Anzahl, die ein Modell in einem Durchlauf verarbeiten kann. Zwischen 8K (klein) und 2M (Experimental).
    \item \textbf{Knowledge Cutoff} Das Datum, bis zu dem das Modell trainiert wurde. Modelle kennen keine Ereignisse nach ihrem Cutoff.
    \item \textbf{Open Weight} Die Modellgewichte sind öffentlich verfügbar, aber die Trainingsdaten sind es nicht.
    \item \textbf{Open Source} Modellgewichte \textit{und} Trainingscode/-daten sind öffentlich. Sehr selten bei großen Modellen.
\end{itemize}

\praxishinweis{Bevor du Modelle vergleichst, kenne deine Anforderungen: Brauchst du Spitzenqualität, minimale Kosten, niedrige Latenz oder Datenhoheit? Keine Benchmark-Tabelle der Welt kann diese Frage beantworten -- nur dein eigener Anwendungsfall.}
...
Die Model-Landschaft hat sich von einem offenen zu einem oligopolistischen Feld entwickelt: drei Akteure (\textbf{OpenAI}, \textbf{Anthropic} und \textbf{Google}) dominieren die Closed-Source-Spitze, während \textbf{Meta (Llama)}, \textbf{Mistral} und \textbf{Qwen} den Open-Source-Markt steuern.
...
Anthropic & Claude 4 Opus & Beste Safety, längster Kontext bei Sonnet \\
...
\bottomrule
\end{tabularx}
\caption{Marktübersicht der wichtigsten Model-Anbieter}
\label{tab:markt}
\end{table}

% ============================================================
% CLOSED SOURCE -- DIE GROSSEN DREI
% ============================================================
...
\praxishinweis{Kein Closed-Source-Modell ist universell überlegen. GPT-4o glänzt bei Geschwindigkeit und Tool-Use, Claude bei Safety und langen Kontexten, Gemini bei Multimodalität. Teste alle drei auf deinen Daten, bevor du dich festlegst.}

% ============================================================
% ARCHITEKTUR -- MULTI-PROVIDER-ROUTING
% ============================================================
...
% ============================================================
% THEORIE -- WARUM ES NICHT EIN BESTES MODELL GIBT
% ============================================================
...
\praxishinweis{Open-Source-Modelle wie Llama oder Mistral sind eine ernsthafte Alternative, nicht nur eine Notlösung. Rechne durch: Ab ca. 1 Mio. API-Anfragen/Monat kann Self-Hosting günstiger sein -- bei voller Datenkontrolle.}
...
\praxishinweis{Multi-Provider ist kein Luxus, sondern betriebliche Notwendigkeit. Ein einziger Provider bedeutet Single Point of Failure -- inklusive Preisrisiko. Drei Provider mit automatischem Fallback sind der Mindeststandard für Produktion.}
...
\praxishinweis{Die Entscheidungsmatrix ist ein Ausgangspunkt, nicht das letzte Wort. Jedes Team sollte ein eigenes Golden Dataset mit 100--500 repräsentativen Queries aufbauen und Modelle regelmäßig dagegen testen. Preise ändern sich, neue Modelle kommen -- deine Evaluierung muss mitwachsen.}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Nutze generische Modell-Namen nicht}: \texttt{gpt-4o} kann sich hinter den Kulissen ändern. Nutze datierte Versionen wie \texttt{gpt-4o-202X-XX-XX}.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Vernachlässigung von Open Source}: Open-Source-Modelle sind so gut, dass sie für viele Use-Cases die API-Kosten ersetzen können -- besonders mit Quantisierung.
...
Llama 4 (70B, lokal) & 0,00 \$ (Strom) & 0,00 \$ & 500--1500 ms & 128K \\
Mistral Large 2 & 2,00 \$ & 6,00 \$ & 600--1200 ms & 128K \\
\bottomrule
\end{tabularx}
\caption{Vollständiger Kosten- und Performance-Vergleich}
...
\begin{itemize}[leftmargin=*]
    ...
    \item Lokales Llama 4 amortisiert sich ab ca. 1 Mio. Anfragen/Monat
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item Llama (Meta), Mistral, Qwen und DeepSeek dominieren den Open-Source-Bereich
...
\begin{itemize}
    ...
    \item \textbf{Open Source ist gleichauf} mit Closed Source in den meisten Domänen.
...
\begin{itemize}[leftmargin=*]
    ...
    \item Ollama Model Library -- \href{https://ollama.ai/library}{ollama.ai/library}
\end{itemize}

\subsection{Tools, die ich nutze}
\begin{itemize}[leftmargin=*]
    \item \textbf{LiteLLM} -- \href{https://github.com/BerriAI/litellm}{github.com/BerriAI/litellm} -- Unified API für 100+ Provider
    \item \textbf{vLLM} -- \href{https://github.com/vllm-project/vllm}{github.com/vllm-project/vllm} -- Hochperformanter Inference-Server
    \item \textbf{Together AI} -- \href{https://together.ai}{together.ai} -- API für Open-Source-Modelle
    \item \textbf{Chatbot Arena} -- \href{https://lmarena.ai}{lmarena.ai} -- Live-Benchmark mit menschlichen Bewertungen
\end{itemize}

\autornotiz{
Nächstes Kapitel: Token-Management und Kosten -- die härteste Engineering-Disziplin.
Da lernst du: Tokenisierung (BPE vs. SentencePiece). Deutsch = 1.4-2x Tokens. Context Window Budgeting. Lost in the Middle. Input vs. Output Pricing. Prompt Caching (KV-Cache). Semantic Cache als größter Hebel. Model Routing (gpt-4o-mini Default, teuer nur bei Bedarf).
}

\textbf{Nächster Schritt:} Mit der Modell-Landschaft im Blick geht es weiter zu Token-Management -- das Fundament aller Kostenkontrolle und Context-Engineering.
\endinput
