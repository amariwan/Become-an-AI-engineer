...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Vier Phasen einer RAG-Pipeline?}

    \textbf{Antwort:} 1) Indexing (Chunken/Embedden). 2) Retrieval (Vektor-Suche). 3) Augmentation (Kontext in Prompt packen). 4) Generation (LLM-Antwort).

    \item \textbf{Cosine Similarity vs MMR (Maximal Marginal Relevance)?}

    \textbf{Antwort:} Cosine liefert hochähnliche, aber oft redundante Ergebnisse. MMR bestraft Redundanz und erzwingt diverse Informationsabdeckung.

    \item \textbf{Schlechte RAG-Antworten — Debugging-Reihenfolge?}

    \textbf{Antwort:} 1) Retrieval prüfen (Hit Rate @ K). 2) Chunk-Qualität sichten. 3) LLM-Prompt isoliert testen. (Der Fehler liegt meist beim Retrieval, nicht beim LLM).

    \item \textbf{HyDE (Hypothetical Document Embeddings)?}

    \textbf{Antwort:} LLM generiert eine fiktive Antwort auf die Query. Diese Antwort (statt der Query) wird geembeddet, um das semantische Gap bei kurzen User-Suchbegriffen zu schließen.

    \item \textbf{Tenant-Isolation in RAG-Systemen sicherstellen?}

    \textbf{Antwort:} Niemals dem LLM überlassen. Harte Metadaten-Filter (Tenant-ID) *vor* der Vector-Similarity-Search auf Datenbankebene anwenden.

    \item \textbf{Cross-Encoder Re-Ranker Architektur?}

    \textbf{Antwort:} Bi-Encoder sucht schnell die Top-100. Cross-Encoder füttert Query+Dokument gemeinsam durch Attention-Layers für hochpräzises, aber rechenintensives Re-Ranking der Top-5.

    \item \textbf{VectorDB wächst auf 100 Mio Chunks, RAM läuft voll. Skalierungs-Strategie?}

    \textbf{Antwort:} Product Quantization (PQ) + Scalar Quantization (SQ) zur 80\%igen RAM-Reduktion, gepaart mit Two-Stage-Retrieval (grob im RAM, exakt auf SSD).

    \item \textbf{Wie wählst du Chunk-Size und Overlap?}

    \textbf{Antwort:} Deterministisch, nicht raten. Chunk-Size basierend auf dem Embedding-Modell-Limit (z.B. 512 Tokens). Overlap (10-20\%) zur Erhaltung des semantischen Kontexts an Schnittkanten.

    \item \textbf{GraphRAG vs VectorRAG: Wann wechselst du?}

    \textbf{Antwort:} VectorRAG versagt bei globalen Zusammenhängen ("Was ist das Leitthema über alle Dokumente?"). GraphRAG extrahiert Entitäten/Beziehungen (Knowledge Graph) für holistische Queries.

    \item \textbf{Selbst-Querying Retriever?}

    \textbf{Antwort:} LLM übersetzt natürliche Sprache ("Zeige PDFs von 2023 über AI") in eine strukturierte Query mit Metadaten-Filtern (`year=2023`, `tag=AI`) *plus* Vector-Search.
