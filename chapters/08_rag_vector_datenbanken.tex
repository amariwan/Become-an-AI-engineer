...
Pre-trained Models haben zwei fundamentale Limitationen:

\begin{enumerate}[leftmargin=*]
    \item \textbf{Kein Firmenwissen}: Modelle kennen keine internen Dokumente, Produktdatenbanken oder Kundendaten.
    \item \textbf{Veraltete Information}: Der Knowledge Cutoff ist real -- Modelle wissen nichts über aktuelle Events, Produkte oder Preisänderungen.
\end{enumerate}

Fine-Tuning könnte beide Probleme lösen, ist aber teuer, aufwendig und unflexibel: Ein fine-getuntes Modell hat nur das Wissen des Trainingszeitpunkts. Neue Produkte bedeuten erneutes Training.

\textbf{RAG (Retrieval-Augmented Generation)} löst beide Probleme anders: Statt Wissen in Modellgewichten zu speichern, wird es bei jeder Anfrage aus einer externen Wissensdatenbank abgerufen und als Kontext in den Prompt eingefügt. Ergebnis: aktuelle, faktenbasierte Antworten -- ohne Fine-Tuning, ohne Modell-Updates, mit vollständiger Kontrolle über die Wissensbasis.

RAG ist das dominierende Pattern für LLM-Produktionssysteme: über 70\,\% aller produktiven LLM-Deployments nutzen RAG.
...
\begin{praxishinweis}
RAG ist kein Fine-Tuning-Ersatz. RAG liefert Kontext \textit{zur Laufzeit}, Fine-Tuning formt Modellgewichte \textit{dauerhaft}. RAG gewinnt, wenn sich das Wissen häufig ändert; Fine-Tuning, wenn Stil oder Verhalten dauerhaft angepasst werden müssen. In der Praxis ergänzen sich beide.
\end{praxishinweis}
...
Cosine Similarity misst Text-Ähnlichkeit, nicht Question-Answering-Relevanz. Ein Chunk über "Rückgabepolitik" kann hohe Similarity zur Frage haben -- aber das Chunk "Versandkosten sind im Preis enthalten" ist für "Wie hoch sind die Versandkosten?" trotzdem irrelevant.

\begin{praxishinweis}
Ein Cross-Encoder Re-Ranker ist der effektivste Einzel-Hebel für Retrieval-Qualität ab 10.000+ Chunks. Bi-Encoder (Embedding-Suche) findet schnell die Top-100; der Cross-Encoder bewertet Query+Dokument gemeinsam durch Attention-Layers und sortiert die Top-5 optimal für den LLM-Kontext.
\end{praxishinweis}
...
\begin{praxishinweis}
Die Chunking-Strategie ist der größte Hebel für RAG-Qualität -- wichtiger als das Embedding-Modell. Falsche Chunk-Größe oder fehlender Overlap ruinieren jedes Retrieval. Start mit Recursive Chunking (512 Tokens, 64 Overlap), dann systematisch messen: Hit Rate @ 5 vor und nach jeder Änderung.
\end{praxishinweis}
...
\midrule
\end{tabularx}
\caption{Embedding-Modelle im Vergleich}
...
\midrule
\end{tabularx}
\caption{Vector Databases im Vergleich}
\label{tab:vectordbs}
\end{table}
...
\begin{praxishinweis}
Jedes Dokument im Vector Store ist ein potenzielles Injection-Vehikel. Ein Angreifer lädt ein Dokument mit "Ignore previous instructions" hoch -- wenn es retrieved wird, gelangt die Injection in den LLM-Kontext. Gegenmaßnahmen: Dokument-Scanning vor Indexierung, Output-Content-Filter, strikte Tenant-Isolation auf Datenbankebene.
\end{praxishinweis}
