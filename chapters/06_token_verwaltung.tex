...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Deutsche RAG: Gleiche Wortzahl, 40 \% mehr Kosten. Warum?}

    \textbf{Antwort:} Tokenizer Bias zulasten von Sprachen mit langen Komposita.

    \item \textbf{4K-Bild mit `detail="high"`: Wie wird abgerechnet?}

    \textbf{Antwort:} Skalierung auf 2048x2048, Zerlegung in 512x512 Kacheln (je 170 Tokens). Teuer.

    \item \textbf{Denial-of-Wallet-Angriff über kurze Prompts. Prävention?}

    \textbf{Antwort:} Angreifer nutzt Prompts wie "Schreibe 50.000 Wörter". Zwingend `max\_tokens` für Output limitieren.

    \item \textbf{History-Kompression bei 8K Tokens: JSON-Format wird ignoriert. Fehler?}

    \textbf{Antwort:} Der System-Prompt wurde fälschlicherweise in die dynamische Kompression (Summary) gezogen. System-Prompt muss isoliert und gepinnt bleiben.

    \item \textbf{`tiktoken` für Anthropic-Budget-Monitoring: Requests blockiert. Warum?}

    \textbf{Antwort:} Tokenizer sind proprietär. Tiktoken (OpenAI) zählt Claude-Tokens falsch (10–20 \% Abweichung).

    \item \textbf{Pretty-Printed JSON im System-Prompt. Bewertung?}

    \textbf{Antwort:} Anti-Pattern. Verbraucht 20–40 \% mehr Tokens durch Leerzeichen. Minifiziertes JSON nutzen.

    \item \textbf{100.000 Rezensionen mit `stream=True` via API. Problem?}

    \textbf{Antwort:} Streaming blockiert Durchsatz. Für asynchrone Massenverarbeitung die Batch API (50 \% Rabatt, 24h Window) nutzen.

    \item \textbf{Kunde X verbraucht globales OpenAI-Limit → Kunde Y kann nicht einloggen. Lösung?}

    \textbf{Antwort:} Multi-Tenant-Isolation via Token Bucket im eigenen API-Gateway, bevor der Request zu OpenAI geht.

    \item \textbf{User sendet Base64-codiertes Bild statt URL an Vision API. Performance-Impact?}

    \textbf{Antwort:} Base64 bläht den Request-Body extrem auf (Bandbreite/Latenz). Bei großen Traffic-Mengen immer pre-signed URLs (z.B. S3) übergeben.

    \item \textbf{Wie trackst du Token-Usage über Microservices hinweg exakt?}

    \textbf{Antwort:} Token-Usage aus der API-Response extrahieren, mit der `trace\_id` taggen und als Custom Metric an Datadog/Prometheus senden.
