...
\label{chap:ai_agenten}

% ============================================================
% AUTOR-NOTIZ STATT LERNZIELE
% ============================================================
\autornotiz{
2023 habe ich meinen ersten Agent in Produktion deployed. Ein Support-Agent mit 4 Tools.
Er lief 3 Wochen stabil -- dann ein Edge-Case: Tool rief sich selbst rekursiv auf.
12\,000 API-Calls in 4 Minuten. \$400 verbrannt. Der Fix: max\_steps=10, strikte Tool-Validierung,
Timeout pro Call. Seitdem: Jeder Agent braucht harte Limits. Keine Ausnahmen.
}
...
\skipbox{
\textbf{TL;DR:} Ein einfacher LLM-Call ist reaktiv: Prompt rein, Antwort raus. Für Klassifikation, Extraktion, Zusammenfassung reicht das. Aber sobald ein System \textbf{mehrere Schritte} ausführen, \textbf{Tools aufrufen} und \textbf{Ergebnisse interpretieren} muss, reicht ein einzelner Prompt nicht.
}
...
\begin{itemize}
    ...
    \item \textbf{Generator} Erzeugt den Output (Code, Text, Entscheidung). Optimiert auf Kreativität und Geschwindigkeit.
    \item \textbf{Evaluator} Prüft den Output gegen externe Kriterien (Tests, Builds, Regeln, messbare Bedingungen). Optimiert auf Korrektheit.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Tool} Eine Funktion, die der Agent aufrufen kann: API, Datenbank, Rechner, Dateisystem. Tool haben eine Signatur (Name, Parameter, Rückgabetyp) und eine Beschreibung für das LLM.
    \item \textbf{Memory} Der Agent muss sich an frühere Schritte erinnern. Kurzzeitgedächtnis = Chat-History, Langzeitgedächtnis = Vector Store.
    \item \textbf{Planning} Der Agent entscheidet, welche Tools in welcher Reihenfolge aufgerufen werden. Bei einfachen Agenten geschieht das implizit (ReAct), bei komplexen explizit (Plan-and-Solve).
    \item \textbf{Orchestration} Bei Multi-Agent-Systemen: Wer ruft wen wann auf? Supervisor-Agenten verteilen Aufgaben an spezialisierte Worker.
\end{itemize}

\praxishinweis{Tool-Beschreibungen sind die halbe Miete. Ein schlecht beschriebenes Tool wird nie aufgerufen. Jede Tool-Definition braucht: präzisen Namen, klaren Einsatzzweck, dokumentierte Parameter mit Beispielwerten. Maximal 5--10 Tools gleichzeitig -- bei mehr: hierarchische Tool-Auswahl.}
...
\begin{itemize}
    ...
    \item \textbf{Agent-Framework} erlaubt das Bauen von Agenten: Tool-Calls, Loops, Planung.
    \item \textbf{AgentOS} betreibt Agenten: Lebenszyklus, Sicherheit, Monitoring, Compliance.
\end{itemize}
...
\label{tab:multiframeworks}
\end{table}

\subsection{Fehlermodi in Multi-Agent-Systemen}

\praxishinweis{Multi-Agent-Systeme multiplizieren Kosten und Latenz: 5 Agenten × 3 Schritte × 4 LLM-Calls = 60 Calls pro Task. Immer Budget-Limits setzen und Modell-Tiering nutzen (günstiges Modell für Routine, teures für Reasoning). Ein einzelner Prompt reicht oft -- Agent nur bei Multi-Step-Aufgaben mit Tool-Bedarf.}
...
% ===========================================================%
% SICHERHEIT (VORWÄRTSVERWEIS)
% ===========================================================%
\section{Sicherheit -- Vorwärtsverweis auf Kap. 16}

\praxishinweis{Agenten sind mächtiger als einfache LLM-Calls -- und damit gefährlicher. Ein Agent ohne max\_steps, Token-Budget und Tool-Whitelist kann Tausende API-Calls auslösen (Denial-of-Wallet). Jedes Tool-Ergebnis ist ein potenzielles Injection-Vehikel. HITL für kritische Aktionen ist keine Option, sondern Pflicht.}

Die vollständige Agent-Security-Strategie (SafetyGuard mit max\_steps/Token-Budget/Tool-Whitelist, HITL-Gates für kritische Aktionen, Tool-Result-Injection-Scanning, Tool-Whitelists mit Domain-Restrictions, Cost-Budget-Guards) findest du in \textbf{Kapitel 16 (Security)}.

Hier nur das Minimum für Agent-Produktion:
\begin{enumerate}
    \item \textbf{SafetyGuard}: max\_steps, Token-Budget, Tool-Whitelist (Default Deny), HITL-Gates für kritische Actions
    \item \textbf{Tool-Result-Scanning}: Jedes Tool-Ergebnis auf Injection-Patterns scannen vor Weitergabe an LLM
    \item \textbf{Domain-Restrictions}: HTTP-Tools nur auf allowlisted Domains
    \item \textbf{Cost-Guard}: Per-Run Cost Budget mit Circuit-Breaker
    \item \textbf{Audit-Log}: Jeder Step (Thought, Action, Observation, Result) für Forensik
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
    ...
    \item OpenAI Assistants API: \href{https://platform.openai.com/docs/assistants/overview}{platform.openai.com/docs/assistants/overview}
...
\autornotiz{
Nächstes Kapitel: Multimodale KI -- Vision, Audio und Video.
Da lernst du: Token-Äquivalenz von Bildern (2000 Tokens für 1024x1024). Vision RAG für Dokumente. Frame-Sampling für Video. OCR-Check vor LLM-Analyse. Multimodale Injection via Bild. Und warum Detail-Level (high/low) Kosten und Qualität steuert.
}

\textbf{Nächster Schritt:} Mit Agenten-Architektur etabliert geht es weiter zu multimodalen Modellen -- wie du LLMs Augen und Ohren gibst.
\endinput
