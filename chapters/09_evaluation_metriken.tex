...
% ============================================================
\autornotiz{
2023 habe ich für einen E-Commerce-Kunden ein Evaluations-Dashboard gebaut. 50k Queries/Tag.
Ohne Golden Set: Jeder Prompt-Change war Blindflug. "Fühlt sich besser an" = keine Metrik.
Mit Golden Set (100 Cases) + Multi-Judge (3 Modelle) + CI-Gate:
- Regression bei Prompt-Change in 2 Min erkannt, nicht in 2 Wochen durch User-Beschwerden.
- Kosten/Query um 34\% gesenkt durch Token-Budget-Check in CI.
- Halluzinationsrate von 8\% auf 1.2\% gedrückt durch Faithfulness-Metrik + Auto-Retry.
...
\begin{itemize}
    ...
    \item \textbf{BLEU (Bilingual Evaluation Understudy)} Misst die N-Gramm-Überlappung zwischen generiertem und Referenztext. Ursprünglich für Maschinenübersetzung entwickelt. Schwäche: Bestraft paraphrasierte, aber korrekte Antworten.
    \item \textbf{ROUGE (Recall-Oriented Understudy for Gisting Evaluation)} Misst Recall der N-Gramm-Überlappung. Verbreitet für Zusammenfassungen. Schwäche: Erfasst keine semantische Äquivalenz.
    \item \textbf{METEOR} Berücksichtigt Synonyme und Wortstellung. Besser als BLEU für einzelne Sätze.
    \item \textbf{BLEURT} Ein lernbares Metrik-Modell, das menschliche Bewertungen approximiert. Robuster als BLEU, aber rechenintensiver.
    \item \textbf{SEMANTIC SIMILARITY} Embedding-basierte Ähnlichkeit zwischen generierter und Referenz-Antwort. Erfasst Paraphrasen, aber kein Faktencheck.
\end{itemize}
...
Gib DEINUR eine Zahl zwischen 1 und 10 zurueck, keine Erklaerungen."""
...
\praxishinweis{LLM-as-a-Judge hat systematische Biases. Ein GPT-4o-Judge bevorzugt lange, ausführliche Antworten; Claude bevorzugt vorsichtige; Gemini bevorzugt Aufzählungen. Gegenmaßnahmen: Multi-Judge (3 Modelle, Majority Vote), Rubric-basierte Bewertung statt freier Skala, Position-Swapping bei Pairwise-Vergleichen.}
...
\begin{itemize}
    ...
    \item \textbf{Devil's Advocate} Greift Annahmen an, sucht Schwachstellen. Testet die Robustheit der Lösung.
    \item \textbf{First Principles} Baut die Antwort von Grundprinzipien aus neu auf. Vermeidet gruppen- oder modelltypische Denkmuster.
    \item \textbf{Optimist} Sucht nach Chancen, Upsides und unerwarteten Vorteilen. Balanciert zu kritische Analysen aus.
    \item \textbf{Executor} Bleibt fokussiert auf Umsetzung, Machbarkeit und konkrete nächste Schritte. Verhindert Theorie-Overload.
    \item \textbf{Outsider} Kommt ohne Domänenwissen. Prüft, ob die Antwort auch für Laien verständlich und plausibel ist.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Evaluation ohne Golden Dataset} Ein LLM-Judge allein ohne feste Testfälle ist keine reproduzierbare Evaluation. Du misst nur die Meinung des Judges, nicht die tatsächliche Qualität.
    \item \textbf{LLM-Judge mit demselben Modell} GPT-4o beurteilt GPT-4o-Antworten. Das ist kein Test -- das ist Selbstbestätigung. Der Judge bevorzugt systematisch Outputs, die dem eigenen Stil ähneln.
    \item \textbf{Nur Accuracy messen} Ein System kann 100\% Accuracy haben und trotzdem unbrauchbar sein: zu langsam, zu teuer, unsicher. Miss immer Qualität, Kosten und Latenz.
    \item \textbf{Einmaliges Dataset} Das Dataset vom ersten Tag wird nach drei Monaten nicht mehr relevant sein. User-Fragen ändern sich, das Produkt ändert sich, die Modelle ändern sich. Pflege das Dataset kontinuierlich.
    \item \textbf{Menschliche Evaluation ignorieren} LLM-Judges sind gut, aber ersetzen keine domänenspezifische menschliche Bewertung. Insbesondere bei medizinischen, rechtlichen oder finanziellen Inhalten.
    \item \textbf{Evaluation in der Produktion vergessen} Offline-Evaluation zeigt, ob ein Prompt gut ist. Aber erst die Online-Metrik zeigt, ob das System im echten Einsatz funktioniert.
\end{itemize}
...
% ============================================================
\section{Codebeispiele -- CI/CD-Integration für Evaluation}
...
\praxishinweis{Evaluations-Pipelines verarbeiten oft sensitive Daten: User-Inputs aus Produktion, interne Dokumente, Ground-Truth-Antworten mit Geschäftsgeheimnissen. Golden Datasets gehören nie in öffentliche Repositories. Separate API-Keys mit Budget-Limits für Evals nutzen, nicht die Produktions-Keys.}
...
% ============================================================
% DIE METRIKEN-ENTSCHEIDUNGSMATRIX
% ============================================================
\section{Die Metriken-Entscheidungsmatrix}

Die wichtigste Frage in der Evaluation ist nicht "Welche Metrik ist am genauesten?" sondern "Welche Metrik zählt für diesen Anwendungsfall?" -- denn die falsche Metrik führt zu optimalem Verhalten für das falsche Ziel.

Die folgende Matrix gibt dir einen schnellen Leitfaden: Welches Use Case hat welche Priorität -- und welches sollte man nie ignorieren.

\begin{table}[H]
\centering
\begin{tabularx}{\linewidth}{l l l l}
\toprule
\textbf{Use Case} & \textbf{Primär} & \textbf{Sekundär} & \textbf{Guardrail} \\
\midrule
Support-Chatbot & Accuracy & P95 Latenz $<$ 2s & Halluzinationen \\
Code-Generation & Functional Correctness & Format Compliance & Security \\
Summarization & Faithfulness & Coverage & PII-Leak \\
RAG-System & Hit-Rate & MRR @ K & Context Precision \\
Agent-System & Success-Rate & Step-Efficiency & Cost/Task \\
\bottomrule
\end{tabularx}
\caption{Metriken-Entscheidungsmatrix -- nach Use Case priorisiert}
\end{table}

\praxishinweis{Beginne mit einer Metrik pro Use Case. Nicht mehr. Wenn du nur eine Zahl aus dem Dashboard behältst, was soll es sein? Für einen Support-Chatbot ist das die Accuracy -- alles andere ist Lärm, wenn die Antworten falsch sind.}

\subsection{Die drei Metrik-Ebenen}

\begin{enumerate}[leftmargin=*]
     \item \textbf{Die Einzelmeterik} -- Was misst ein einzelner Request? (Accuracy, Latenz, Kosten)
     \item \textbf{Die Systemmeterik} -- Was zeigt das Gesamtsystem? (P95 Latenz, Error Rate, Kosten/Tag)
     \item \textbf{Die Business-Metrik} -- Was zählt für das Geschäft? (Customer Satisfaction, Conversion, Cost per Ticket)
\end{enumerate}

\realitycheck{Die meisten Teams scheitern nicht an der Einzelmeterik -- sie scheitern an der Business-Metrik. Ein System kann 99\,\% Accuracy haben und trotzdem 80\,\% der Kunden frustrieren, wenn die 1\,\% falschen Antworten katastrophal sind.}

\subsection{Code-Qualitäts-Kennzeichnung}

Jeder Code-Block in diesem Buch ist nach seiner Produktionsreife gekennzeichnet:

\begin{itemize}[leftmargin=*]
     \item \textbf{[Production-Ready]} -- Direkt in der Produktion verwendbar. Includes error handling, retry logic, and cost monitoring.
     \item \textbf{[Didaktisch vereinfacht]} -- Lernzwecke optimiert.生产-ready Erweiterungen als \textit{Aufgabe} gekennzeichnet.
     \item \textbf{[Architekturskizze]} -- Nur konzeptionell. Implementierungsdetails folgen im nächsten Kapitel.
\end{itemize}
...
\par\mbox{}\par
\begin{merkeEnv}
Metriken ohne Use Case sind nur Zahlen. Finde deine eine Nummer, die zählt.
\end{merkeEnv}
...
\textbf{Nächster Schritt:} Jetzt weißt du, wie du misst. Im nächsten Kapitel geht es um Inference Optimization and Serving -- wie du dein System schnell und kostengünstig machst.
\endinput
