...
\label{chap:rag_vector_datenbanken}
...
\medskip
\textbf{Messrahmen der RAG-Evolution:}
\begin{itemize}[leftmargin=*]
  \item \textbf{Metric:} Hit Rate @ 5, MRR @ 5, Faithfulness, Answer Relevance, p99 Latency, Cost/Query
  \item \textbf{Baseline:} Naive RAG (text-embedding-ada-002, Pinecone, top-k=5, no re-rank)
  \item \textbf{Dataset:} 50k product docs (DE), 200k real user queries, stratified sample n=1.000
  \item \textbf{n:} 1.000 queries (200 per intent: status, billing, technical, cancellation, OOD)
  \item \textbf{Metric:} Hit Rate @ 5: 67\% $\to$ 94\%, MRR @ 5: 0.42 $\to$ 0.81, Faithfulness: 0.58 $\to$ 0.91, p99: 3.2s $\to$ 800ms, Cost/Query: \$2.25 $\to$ \$0.19
  \item \textbf{What Changed:} Chunking (recursive 512/64), Hybrid Search (BM25+embedding), Re-Ranker (cross-encoder/ms-marco-MiniLM-L-6-v2), pgvector + HNSW
  \item \textbf{Limitations:} Single tenant, no A/B (sequential), query distribution shift (seasonal), embedding model update mid-period
\end{itemize}
}
...
% ============================================================
% SICHERHEIT -- INDIRECT PROMPT INJECTION (VORWÄRTSVERWEIS)
% ============================================================
\section{Sicherheit -- Indirect Prompt Injection (Vorwärtsverweis auf Kap. 16)}
...
Die vollständige RAG-Security-Strategie (Input-Scanning vor Indexing, Tenant-Isolation via Row-Level-Security, Output-Filter, PII-Masking vor Embedding, Document-Classification, Access-Control, Audit-Log) findest du in \textbf{Kapitel 16 (Security)}.

Hier nur das Minimum für RAG-Produktion:
\begin{enumerate}
    \item \textbf{Input-Scanning} vor Indexing (Regex + LLM-Judge für verdächtige Chunks)
    \item \textbf{Tenant-Isolation} auf DB-Ebene (Row-Level-Security in PostgreSQL/pgvector)
    \item \textbf{Output-Filter} nach LLM-Generation (Regex + LLM-Judge auf Leaked Content)
    \item \textbf{PII-Masking} \textbf{vor} Embedding (Presidio/Microsoft Presidio)
    \item \textbf{Audit-Log} mit User-ID, Query, Retrieved Chunks, Response
\end{enumerate}
