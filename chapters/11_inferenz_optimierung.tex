...
\label{chap:inference_optimierung}

% ============================================================
% AUTOR-NOTIZ: Self-Hosting Story (NEU)
% ============================================================
\autornotiz{
2024: SupportPilot musste PII-Tickets (Kundendaten, Kreditkarten) verarbeiten. Cloud-APIs (OpenAI, Anthropic) schieden aus Compliance-Gründen aus.
Lösung: Llama 3.1 70B auf 4x A100 (vLLM, AWQ-INT4).
Erster Versuch: FP16, Batch=1. TTFT 1.2s, TPOT 65ms, Durchsatz 15 Tok/s. GPU-Auslastung 50\%.
Nach Optimierung: AWQ-INT4, Continuous Batching (Batch=32), Speculative Decoding (Draft: Llama-3.2-1B), Prefix Caching.
TTFT 850ms, TPOT 18ms, Durchsatz 320 Tok/s. GPU-Auslastung 95\%. Kosten: Strom statt \$3k/Monat API.
Der Unterschied war nicht das Modell. Es war Inference-Engineering.
}
...
\begin{itemize}
    ...
    \item \textbf{Prefill-Phase} Der gesamte Input (Prompt) wird parallel verarbeitet. Das Modell berechnet die Key-Value-Caches (KV-Cache) für alle Token gleichzeitig. Ergebnis: Ein einzelner Output-Token.
    \item \textbf{Decode-Phase} Token für Token wird sequentiell generiert. Jeder Schritt nutzt den KV-Cache aus der Prefill-Phase. Der Engpass ist hier die Speicherbandbreite, nicht die Rechenleistung.
\end{itemize}
...
\protect\label{tab:inference-engpaesse}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{TTFT (Time to First Token)} Zeit vom Request-Eingang bis zum ersten Output-Token. Entscheidend für User-Experience bei Streaming. Ziel: $<$ 500ms.
    \item \textbf{TPOT (Time per Output Token)} Zeit pro generiertem Token im Decode-Schritt. Bestimmt die Gesamtlatenz. Ziel: $<$ 30ms.
    \item \textbf{Durchsatz (Throughput)} Tokens pro Sekunde \textit{über alle Requests hinweg}. Entscheidend für Batch-Verarbeitung. In Tokens/s oder Requests/s gemessen.
    \item \textbf{VRAM-Auslastung} Belegter Speicher durch Modellgewichte + KV-Cache + Overhead. Bestimmt die maximale Batch-Size.
\end{itemize}

\praxishinweis{TTFT und TPOT sind die zwei Metriken, die User wirklich spüren. TTFT $<$ 500ms für Streaming, TPOT $<$ 30ms pro Token. Misst immer den P95-Wert -- der Durchschnitt lügt bei Latenz.}
...
vLLM (\href{https://github.com/vllm-project/vllm}{github.com/vllm-project/vllm}) ist der dominierende Open-Source-Inference-Server. Seine Kerninnovation ist \textbf{PagedAttention} -- eine Speicherverwaltung für den KV-Cache, inspiriert von virtuellen Speicherseiten in Betriebssystemen.
...
\praxishinweis{Continuous Batching ist kein magischer Turbo. Für interaktive Chat-Anwendungen kleine Batches (4-8) wählen, sonst leidet die TTFT. Für Batch-Jobs 32-64. Und immer \texttt{--enable-prefix-caching} aktivieren.}
...
\begin{itemize}
    ...
    \item \textbf{GPTQ (Post-Training)} Kalibriert die Quantisierung mit einem kleinen Kalibrierungs-Dataset. Gut für GPU-Inference. Unterstützt von vLLM und AutoGPTQ.
    \item \textbf{AWQ (Activation-Aware)} Berücksichtigt die Aktivierungsverteilung, nicht nur die Gewichte. Erhält mehr Qualität als GPTQ bei gleicher Bitrate. Der empfohlene Standard für GPU-Inference.
    \item \textbf{GGUF (llama.cpp)} Format für CPU- und Mixed-Inference. Erlaubt heterogenes Computing (GPU + CPU). Standard für lokale und On-Device-Inference via Ollama, LM Studio.
    \item \textbf{Bitsandbytes (NF4)} Speziell für QLoRA-Fine-Tuning entwickelt. 4-Bit-NormalFloat-Format, optimiert für normalverteilte Gewichte.
\end{itemize}
...
\praxishinweis{Quantisierung nie blind deployen. Ein INT4-Modell mag im MMLU 2\,\% verlieren -- aber in deiner spezifischen Domäne 15\,\%. Golden Dataset auf FP16 und INT4 evaluieren -- erst dann entscheiden. Für Code und Mathematik lieber INT8 nehmen.}
...
\paragraph{Voraussetzung:} Draft-Modell muss die gleiche Tokenizer-Vokabular haben wie das Target-Modell. In vLLM integriert:
...
\praxishinweis{Speculative Decoding bringt 2-3x Speedup -- aber nur wenn Draft und Target denselben Tokenizer teilen. Prüft vor dem Deployment die Akzeptanzrate: unter 60\,\% lohnt sich der Overhead nicht.}
...
\praxishinweis{Inference-Server sind kritische Infrastruktur. Immer hinter API-Gateway betreiben, Rate-Limiting aktivieren, Audit-Log führen. Und niemals \texttt{--gpu-memory-utilization 1.0} setzen -- der KV-Cache braucht Reserve für Spitzen.}
...
\begin{itemize}[leftmargin=*]
    ...
    \item vLLM mit PagedAttention ist der Standard-Server -- bis zu 60\,\% weniger KV-Cache-Fragmentierung
...
\textbf{Nächster Schritt:} Inference optimiert, Self-Hosting läuft -- jetzt die Entscheidung: Fine-Tuning oder RAG? LoRA/QLoRA, Forgetting-Check, Multi-LoRA-Serving. Wann lohnt sich Gewichts-Anpassung vs. Kontext-Anreicherung.
\endinput
