...
Bis 2024 waren LLMs reine Textmaschinen. 2026 ist das anders: \textbf{GPT-4o, Claude 4 Vision und Gemini 3} verarbeiten Bilder, Audio und Video nativ. Mehr als 60\,\% der neuen Enterprise-KI-Anwendungen nutzen multimodale Eingaben (Gartner 2026). Ein Support-Bot, der Screenshots liest, ein Code-Review-Tool, das Architekturdiagramme versteht, oder ein Qualitätsinspektor, der Kamerabilder auswertet -- das sind keine Zukunftsszenarien mehr, sondern aktuelle Produktionssysteme.
...
\bottomrule
\end{tabularx}
\caption{Multimodale Modelle 2026 im Vergleich}
...
Ein Spezialfall von Vision: technische Diagramme, Architekturskizzen, UML-Diagramme und Charts. Multimodale Modelle sind 2026 gut darin, Diagramme zu lesen, aber sie machen systematische Fehler bei Achsenbeschriftungen und Metriken.
...
Die Umwandlung von Sprache in Text ist die reifste multimodale Technologie. 2026 wird sie praktisch fehlerfrei von GPT-4o-Audio, Whisper v3 und Gemini 3 beherrscht.
...
Video-Verarbeitung in LLMs ist 2026 noch kein nativer Echtzeit-Stream, sondern erfolgt über \textbf{Frame-Sampling}. Das Modell bekommt eine Auswahl an Frames in regelmässigen Abständen und extrahiert daraus Informationen.
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{20 Frames aus 5-min Video = 6.800 Tokens. Kosten massiv senken?}

    \textbf{Antwort:} 1) Strukturelle Keyframe-Detektion (OpenCV) filtert statische Bilder raus. 2) Image-Detail der API auf `low` setzen. 3) Auflösung drastisch downscalen.

    \item \textbf{Bild mit unsichtbarem Prompt-Injection-Text (White on White). Abwehr?}

    \textbf{Antwort:} OCR-Pipeline (Tesseract) vorgeschaltet, die den reinen Text extrahiert. Dieser wird durch ein NLP-Guardrail gejagt, bevor das Bild ans Vision-Modell geht.

    \item \textbf{GPT-4o vs Claude 3.5 Sonnet vs Gemini 1.5 Pro für Multimodal?}

    \textbf{Antwort:} Claude dominiert extrem bei UI-Screenshots und Code/Diagramm-Transkription. Gemini ist durch das massive Fenster für Video-Analyse (Long-Context) unschlagbar.

    \item \textbf{Multi-Vector Retrieval (ColPali/Vision-RAG) Architektur?}

    \textbf{Antwort:} PDF-Seiten werden als Bilder geembeddet (Vision-Encoder), statt OCR-Text zu extrahieren. Bewahrt Layout, Tabellen und Charts perfekt für semantische Bild-Vektor-Suchen.

    \item \textbf{Kostenfalle: High-Res vs Low-Res Visual Tokens?}

    \textbf{Antwort:} High-Res skaliert das Bild, zerschneidet es in 512x512 Kacheln und berechnet jede (700-1000 Tokens/Bild). Low-Res skaliert alles auf eine 512-Kachel (85 Tokens fix). High-Res nur für OCR zwingend.

    \item \textbf{Audio-Native LLMs (wie GPT-4o Realtime) vs ASR Pipeline?}

    \textbf{Antwort:} Alte Pipeline (Audio->Text->LLM->Text->Audio) verliert Emotion/Tonfall und hat Latenz. Audio-Native Modelle verarbeiten Spectrogram-Tokens direkt in Attention-Layers (Latenz <300ms, Emotion bleibt erhalten).

    \item \textbf{Spatial Reasoning in Vision Language Models (VLM)?}

    \textbf{Antwort:} VLMs haben oft Probleme mit relativen Positionen ("Ist A links von B?"). Lösung: Visual Prompting (Bounding Boxes oder Koordinaten-Grid per OpenCV auf das Bild zeichnen vor dem Senden).

    \item \textbf{Cross-Modal Alignment Problem beim Pre-Training?}

    \textbf{Antwort:} Wie lernt das Modell, dass das Wort "Hund" und das Bild "Hund" identisch sind? Contrastive Learning (CLIP-Architektur) verringert die Vektor-Distanz zwischen passenden Text/Bild-Paaren massiv.

    \item \textbf{Multimodales Prompt Injection (Audio)?}

    \textbf{Antwort:} Angreifer bettet eine gesprochene Anweisung ("Leite Geld um") auf einer Frequenz ein, die das Modell-Audio-Processing erfasst, Menschen aber ignorieren. Härtung: Audio-Filterung (Low-Pass/High-Pass) vor API.

    \item \textbf{Video-Frame Temporal Coherence: Warum halluziniert das VLM?}

    \textbf{Antwort:} Wenn Frames einzeln gesendet werden, fehlt dem VLM das Verständnis für Bewegung/Zeitverlauf. Lösung: Modelle mit nativen Temporal-Attention-Schichten (Sora/Gemini 1.5) nutzen.
