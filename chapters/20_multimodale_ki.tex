...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Vergleiche die Vision-Fähigkeiten von GPT-4o, Claude 4 Vision und Gemini 3. Wann setzt du welches Modell ein?}

    \textbf{Antwort:} GPT-4o: beste Kosten-Effizienz, breite Modalitätsunterstützung (Audio). Claude 4 Vision: beste Diagramm-/Code-Analyse, grösste Auflösung. Gemini 3: nativ bestes Video-Verständnis, niedrigster Token-Preis. Auswahl nach primärer Modalität: Diagramme $\rightarrow$ Claude 4, Video $\rightarrow$ Gemini, General Purpose $\rightarrow$ GPT-4o.

    \item \textbf{20 Frames aus einem 5-minütigen Video erzeugen ein gutes Verständnis, aber kosten 6.800 Tokens für eine Analyse. Wie reduzierst du die Kosten um 60\,\%?}

    \textbf{Antwort:} (1) Keyframe-Detektion statt Fixed-Interval: nur Frames mit Szene-Wechsel. (2) Hierarchische Analyse: 5 Segmente separat mit je 2 Frames analysieren, dann Zusammenfassung. (3) \texttt{detail="low"} statt high. (4) Frame-Grösse auf 512$\times$512 reduzieren. Kombiniert spart das 60-80\,\% der Kosten.

    \item \textbf{Ein User lädt ein Bild mit unsichtbarem Prompt-Injection-Text hoch. Wie verteidigst du dich?}

    \textbf{Antwort:} Zweistufig: (1) OCR auf dem Bild vor LLM-Processing, extrahierten Text durch Input-Guard jagen. (2) Das Bild im Prompt oberhalb des User-Texts platzieren, damit der LLM den Kontext als Systemanweisung erkennt. Zusätzlich: Output-Entropy-Monitoring -- ein plötzlicher Themenwechsel deutet auf erfolgreiche Injection hin.
\end{enumerate}
