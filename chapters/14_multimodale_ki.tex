...
% ============================================================
% AUTOR-NOTIZ: Multimodal Story (NEU)
% ============================================================
\autornotiz{
2024: SupportPilot erhielt Screenshots von Fehlermeldungen (User: "Seht ihr den Fehler?"). Text-only Bot versagte.
Lösung: GPT-4o Vision für Screenshot-Analyse → Fehlercode extrahieren → Knowledge Base Query → Antwort.
Erster Versuch: Alle Screenshots an GPT-4o. Kosten \$0.08/Screenshot, p99 3.2s.
Optimierung: OCR-Vorverarbeitung (Tesseract) → nur relevante Bildausschnitte → GPT-4o-mini Vision.
Kosten \$0.012/Screenshot, p99 1.1s. OCR-Fallback bei Low-Confidence.
Der Unterschied war nicht das Vision-Modell. Es war Preprocessing + Model-Routing.
}
...
Gemini 2.5 Pro & Text, Bild, Audio, Video & nat. Auflösung & 1,25 \$/M Tokens \\
Llama 4 90B & Text, Bild & 1024$\times$1024 & Open Source \\
\bottomrule
\end{tabularx}
\caption{Multimodale Modelle im Vergleich}
\label{tab:multimodalmodels}
\end{table}
...
\praxishinweis{Bild-Tokens summieren sich schneller als gedacht. Ein einzelnes 4K-Bild kostet bei GPT-4o high-detail rund 7.000 Tokens -- mehr als dieser gesamte Absatz. Vorverarbeitung lohnt: zuschneiden, skalieren, Rauschen reduzieren.}
...
\praxishinweis{Vision RAG lohnt sich ab 50+ Seiten pro Tag. Für kleine Dokumentenmengen reicht es, Bilder direkt per Vision-LLM zu analysieren. Die Embedding-Infrastruktur (Vector-DB, Chunking, Indexing) amortisiert sich erst ab Volumen.}
...
Die Umwandlung von Sprache in Text ist die reifste multimodale Technologie. Sie wird praktisch fehlerfrei von GPT-4o-Audio, Whisper v3 und Gemini 2.5 beherrscht.
...
GPT-4o und Gemini 2.5 verarbeiten Audio direkt, ohne separaten STT-Schritt. Das Modell bekommt die Roh-Audiodaten und kann sowohl Inhalt als auch Tonfall, Emotionen und Sprecherwechsel erkennen.
...
\praxishinweis{Audio-Native-Verarbeitung (GPT-4o-Audio, Gemini) erhält Emotion und Tonfall, kostet aber mehr Tokens als STT-$\rightarrow$Text-$\rightarrow$LLM. Entscheide nach Use-Case: Sentiment-Analyse braucht natives Audio, Transkription reicht STT.}
...
\praxishinweis{Frame-Sampling ist ein Kosten-Kompromiss. 1 Frame/s für Szenen-Wechsel, 1 Frame/10s für statische Präsentationen. Und immer low-detail für Video-Frames -- high-detail kostet 4x und bringt bei Bewegung kaum Mehrwert.}
...
% ===========================================================%
% SICHERHEIT MULTIMODAL (VORWÄRTSVERWEIS)
% ===========================================================%
\section{Sicherheit multimodaler Systeme (Vorwärtsverweis auf Kap. 16)}

Multimodale Eingaben eröffnen neue Angriffsvektoren (Prompt Injection via Bild/OCR, Adversarial Patches, Acoustic Attacks). Die vollständige Multimodal-Security-Strategie (OCR-Injection-Check, Adversarial Patch Detection, Audio-Frequency-Filter, Model Pinning für Vision-Encoder) findest du in \textbf{Kapitel 16 (Security)}.

Hier nur das Minimum:
\begin{enumerate}
    \item \textbf{OCR-Injection-Check}: Jedes Bild vor LLM-Call durch Tesseract/PaddleOCR + Regex/Regex-Judge auf Injection-Patterns prüfen
    \item \textbf{Adversarial Patch Detection}: CLIP-basierte Anomalie-Detektion auf Bild-Patches
    \item \textbf{Audio-Frequency-Filter}: Ultraschall-Filter (>20kHz) vor STT-Pipeline
    \item \textbf{Model Pinning}: Vision-Encoder-Version explizit pinnen (keine stillen Provider-Updates)
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
    \item Multimodale Modelle (GPT-4o, Claude 4 Vision, Gemini 2.5) verarbeiten Text, Bilder, Audio und Video in einem Modell.
...
\textbf{Nächster Schritt:} Mit multimodalen Modellen ausgestattet geht es weiter zur zweiten Optimierungsstufe -- Caching, Routing und Guardrails. Wie du wiederholte und ähnliche Anfragen eliminierst und dein System gegen Injection und PII-Leaks absicherst.
\endinput
