...
% ============================================================
\autornotiz{
2024 habe ich für einen FinTech-Kunden ein SQL-Generierungs-Modell fine-getunt. Base: Llama 3.1 8B. LoRA r=16, 500 Examples (Text-to-SQL).
Vorher: 61\% Execution Accuracy auf Spider Test-Set. Nachher: 89\%.
Aber: MMLU-General-Drift -12\%. Fix: 20\% generische Instruct-Examples im Training-Set beibehalten.
Lektion: Fine-Tuning ohne Forgetting-Check ist blind. Eval vor/nach ist Pflicht, nicht Nice-to-have.
}
...
\praxishinweis{Die beste Architektur kombiniert alle drei: Prompt Engineering für den System-Prompt, RAG für aktuelle Fakten, Fine-Tuning für Format und Stil. Nicht entweder-oder, sondern alle drei geschichtet.}
...
Bei einem typischen LLM mit 4.096 Hidden-Dimension und LoRA-Rang~16:
...
\praxishinweis{QLoRA ist der Game-Changer für Teams ohne GPU-Cluster. 70B-Modell auf einer A80 fine-tunen -- aber aufgepasst: Das Basismodell in 4-Bit bedeutet leichten Qualitätsverlust. Für maximale Quality lieber 8B in FP16 mit LoRA.}
...
\protect\label{tab:datasetquality}
\end{table}

\praxishinweis{500 qualitätsgeprüfte Beispiele schlagen 5.000 synthetische. Jeder Fehler im Dataset wird vom Modell gelernt. Manuelle Prüfung von 20\,\% aller Einträge ist Pflicht -- besonders bei synthetisch generierten Daten.}
...
\begin{itemize}
    ...
    \item \textbf{Task-Performance} Wie gut ist das Modell in der \textit{neuen} Aufgabe? Gemessen gegen das Test-Set.
    \item \textbf{Catastrophic Forgetting} Wie stark hat das Modell \textit{alte} Fähigkeiten verloren? Gemessen gegen ein General-Benchmark (MMLU, HumanEval).
\end{itemize}
...
\praxishinweis{Forgetting-Check ist kein Nice-to-have. Ein FT-Modell, das auf SQL glänzt aber "Was ist 2+2?" nicht mehr beantwortet, ist unbrauchbar. Fix: 20-30\% generische Prompts im Dataset behalten.}
...
\praxishinweis{Multi-LoRA-Serving ist der effizienteste Weg, mehrere spezialisierte Modelle zu betreiben. Das Basismodell bleibt 1x im VRAM, Dutzende Adapter teilen sich den Rest. Ideal für SQL-, E-Mail- und Code-Agents auf einer GPU.}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Dataset-Qualität vor Quantität}: 500 hochwertige Beispiele sind besser als 5.000 synthetische.
...
\textbf{Nächster Schritt:} Modell angepasst, Evaluation grün -- jetzt Deployment: FastAPI, Docker, Kubernetes, Canary, Circuit Breaker, Multi-Region Failover. Wie du SupportPilot in die Welt bringst ohne Downtime.
\endinput
