...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Ein Kunde will Llama-3 mit PDF-Handbüchern fine-tunen für Wissensabfragen. Warum rätst du ab?}

    \textbf{Antwort:} Fine-Tuning passt Stil/Format an, ist aber unzuverlässig für neues Faktenwissen (Halluzinationen). RAG ist hier der korrekte Architektur-Ansatz.

    \item \textbf{Du nutzt ein 1M-Token Fenster. Das Modell ignoriert Fakten aus der Mitte des Prompts. Welches Phänomen?}

    \textbf{Antwort:} "Lost in the Middle". LLMs fokussieren sich auf den Anfang und das Ende. Lösung: Re-Ranking nutzen, um nur die relevantesten 10 Dokumente zu übergeben.

    \item \textbf{Gesundheits-Startup (Air-Gapped) braucht ein lokales LLM, hat aber nur eine GPU. Strategie?}

    \textbf{Antwort:} Stark quantisierte Open-Weights (INT4/INT8) wie Llama 3 8B. So bleiben die Patientendaten lokal und das Modell passt in den VRAM einer Standard-GPU.

    \item \textbf{Kosten um 70 \% senken ohne Qualitätsverlust. Ansatz?}

    \textbf{Antwort:} Semantic Router. Ein kleines Modell (GPT-4o-mini) klassifiziert die Komplexität der Anfrage. Nur schwere Tasks gehen an das teure Flagship-Modell.

    \item \textbf{"gpt-4o" ist im Code verdrahtet. Qualität sinkt plötzlich ohne Codeänderung. Ursache?}

    \textbf{Antwort:} Provider-seitiges Modell-Update. Lösung: In Produktion immer feste Version-Pins (z.B. gpt-4o-2026-05-13) nutzen.

    \item \textbf{Lokal gehostetes Modell ist bei deutschen Texten viel langsamer als bei englischen. Warum?}

    \textbf{Antwort:} Tokenizer Bias (Byte-Pair Encoding). Deutsche Komposita zerfallen in mehr Tokens. Abhilfe: Modelle mit multilingualem Tokenizer wählen.

    \item \textbf{Modell schlägt alle Benchmarks, versagt aber bei euren internen Daten. Grund?}

    \textbf{Antwort:} Data Contamination. Die öffentlichen Benchmarks waren Teil der Pre-Training-Daten. Das Modell hat auswendig gelernt, nicht generalisiert.

    \item \textbf{10.000 Tokens System-Prompt bei jedem Agent-Call. Wie senkst du Kosten?}

    \textbf{Antwort:} Prompt Caching auf API-Ebene oder Prefix Caching in vLLM. Der Präfix muss exakt identisch bleiben.

    \item \textbf{Lokal gehostetes 70B-Modell liefert nur 15 Tokens/s. Wie beschleunigen ohne Hardware-Upgrade?}

    \textbf{Antwort:} Speculative Decoding. Ein kleines 8B-Modell entwirft Tokens, das große 70B-Modell verifiziert sie parallel in einem Batch.

    \item \textbf{Chain-of-Thought-Prompt auf winziges 3B-Modell migriert. Modell halluziniert. Warum?}

    \textbf{Antwort:} Fehlen von "Emergent Abilities". Echtes, mehrschrittiges Reasoning entsteht erst ab einer bestimmten Parameterzahl. Kleine Modelle imitieren nur Syntax.
