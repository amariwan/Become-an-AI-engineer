...
% ============================================================
\begin{praxishinweis}
Lernziele sind kein Selbstzweck. Markiere dieses Kapitel als Referenz -- du wirst bei jedem neuen Projekt die Modell-Typologie und die Trade-off-Tabellen brauchen.
\end{praxishinweis}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Adapter / LoRA}: Eine parametereffiziente Variante des Fine-Tunings. Statt aller Gewichte werden nur kleine Adapter-Module trainiert. Die Basisgewichte bleiben eingefroren. Standard.
...
\begin{praxishinweis}
Starte immer mit Prompt-Tuning. Erst wenn das Modell die Aufgabe trotz sauberer Prompts nicht löst, zu Fine-Tuning oder RAG greifen. Fine-Tuning ist teuer und wartungsintensiv -- der meiste Mehrwert entsteht durch bessere Prompts.
\end{praxishinweis}
...
\begin{praxishinweis}
Base Models nie direkt nutzen -- sie sind Rohmaterial für Fine-Tuning. Instruction-Tuned ist der Standard für fast alle Anwendungen. Reasoning Models nur für komplexe Logik, nicht für einfache Extraktion.
\end{praxishinweis}
...
Nicht alle Foundation Models sind gleich. Es hat sich eine klare Typologie
herausgebildet, die bestimmt, welches Modell für welche Aufgabe geeignet ist.
Die Wahl des falschen Modelltyps ist einer der häufigsten Kosten- und Qualitätsfehler.
...
Base Models sind selten im direkten Einsatz. Sie dienen als
...
Dies ist der mit Abstand häufigste Modelltyp im AI Engineering.
...
% ============================================================
\begin{praxishinweis}
Die Modellkategorie bestimmt die Architektur. Ein Reasoning Model für simple Extraktion ist teuer und langsam. Ein SLM für komplexe Logik halluziniert. Wähle die kleinste Kategorie, die die Aufgabe löst.
\end{praxishinweis}
...
\begin{description}[leftmargin=0pt]
    \item[Halluzinationen:] Die Modelle erfinden Fakten mit absolutem Selbstvertrauen. Verbessert durch System-Prompts und RAG, aber nie vollständig eliminiert.
    ...
    \item[Knowledge Cutoffs:] Ein Modell kennt keine Ereignisse nach seinem Knowledge Cutoff. Das ist ein fundamentales Problem für aktuelle Wissensabfragen.
...
\begin{praxishinweis}
Jede LLM-Architektur muss drei Dinge adressieren: Halluzinationen (RAG + Validierung), Knowledge Cutoffs (Datenaktualität sicherstellen) und Kontextgrenzen (Chunking + Re-Ranking). Wer das ignoriert, baut eine KI, die in Produktion scheitert.
\end{praxishinweis}
...
\begin{praxishinweis}
Die teuerste Komponente ist oft nicht das Modell, sondern überflüssige Tokens. Jeder Prompt gehört getrimmt. Ein System-Prompt mit 500 statt 2000 Tokens spart 75\,\% Kosten -- bei gleicher Qualität.
\end{praxishinweis}
