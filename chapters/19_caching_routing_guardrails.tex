...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Erkläre den Unterschied zwischen Response Caching und Semantic Caching. Wann setzt du welches ein?}

    \textbf{Antwort:} Response Cached speichert exakte Input-Output-Paare (Hash-basiert). Semantic Cached speichert Embedding-Vektoren und findet semantisch ähnliche Anfragen. Response Caching für identische Wiederholungen (Bestellstatus), Semantic Caching für variierende Formulierungen (Support-Fragen).

    \item \textbf{Ein User bekommt eine Antwort aus dem Semantic Cache, die sich auf einen \textit{anderen} User bezieht. Was ist passiert?}

    \textbf{Antwort:} Die PII-Redaktion wurde \textit{nach} dem Caching durchgeführt, nicht davor. User-spezifische Daten (Name, Bestellnummer) wurden im Cache gespeichert und an einen anderen User ausgeliefert. Lösung: PII vor dem Caching redigieren und personalisierte Felder vom Caching ausschliessen.

    \item \textbf{Dein Semantic Router routet 20\,\% aller Anfragen falsch. Was machst du?}

    \textbf{Antwort:} (1) Intent-Definitionen prüfen und ggf. trennen. (2) Confidence-Score einführen: Bei Score $<$ 0,7 Fallback-Router mit GPT-4o-mini. (3) Embedding-basierten Router (MiniLM) statt LLM-basiertem Router testen. (4) Falsch klassifizierte Beispiele sammeln und in den Router integrieren.

    \item \textbf{Wie stellst du sicher, dass ein Cache-Eintrag noch gültig ist?}

    \textbf{Antwort:} TTL (Time-to-Live) für jeden Cache-Eintrag. FAQ: 24h, Status: 5 Min, Preise: 1h. Bei Prompt-Änderungen: Cache-Version im Key (z.\,B. \texttt{prompt\_v2\_<hash>}). Bei Modellwechsel: komplette Cache-Invalidierung.

    \item \textbf{Wie berechnest du die Kosteneinsparung durch Caching?}

    \textbf{Antwort:} Kosten ohne Cache: Requests $\times$ Cost pro Request. Kosten mit Cache: (Cache-Hits $\times$ 0) + (Cache-Misses $\times$ Cost pro Request) + Embedding-Kosten. Einsparung: (Hits / Total) $\times$ Total Cost. Beispiel: 30\,\% Hit-Rate bei 10.000 Requests/Tag zu 0,002 EUR = 6 EUR/Tag Ersparnis.

    \item \textbf{Wie schützt du ein LLM-System vor einem Denial-of-Wallet-Angriff?}

    \textbf{Antwort:} Drei Ebenen: (1) Rate-Limiting pro User/IP (60 Requests/min). (2) Budget-Limit pro User/Tag (5 EUR). (3) Globales Budget (100 EUR/Tag). Bei Überschreitung: Request blockieren, Alarm auslösen, Admin benachrichtigen. Zusätzlich: Input-Länge limitieren (4.096 Zeichen) und teure Modelle nur für autorisierte User.
\end{enumerate}
